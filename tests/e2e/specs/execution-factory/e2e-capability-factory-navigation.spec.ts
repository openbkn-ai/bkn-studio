...
test.describe("Capability Factory navigation", () => {
  test("user can discover management, market, and sandbox runtime from the official entry", async ({
    ...
    await page.route("**/api/agent-operator-integration/v1/sandbox/health", async (route) => {
    ...
    await page.route("**/api/agent-operator-integration/v1/sandbox/pool", async (route) => {
      sandboxRequests.push(route.request().url());
      await route.fulfill({
        contentType: "application/json",
        json: {
    ...
    await page.route("**/api/agent-operator-integration/v1/sandbox/sessions**", async (route) => {
      sandboxRequests.push(route.request().url());
      await route.fulfill({
        contentType: "application/json",
        json: {
    ...
    await page
      .getByText(/沙箱运行时管理|Sandbox Runtime/i)
      .first()
      .click();
    await expect(page).toHaveURL(/\/studio\/execution-factory\/sandbox-runtime/);
    await expect(page.getByText(/沙箱运行时管理|Sandbox Runtime/i).first()).toBeVisible();
    await expect(
      page.getByText(/加载沙箱运行时数据失败|Failed to load sandbox runtime/i),
    ).toHaveCount(0);
