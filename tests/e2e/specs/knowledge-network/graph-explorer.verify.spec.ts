...
test("graph explorer end to end", async ({ page }) => {
  test.setTimeout(240_000);

  // 1. Open the explorer by URL; the workspace has no button for it yet.
  const explorerPath = `/studio/knowledge-network/workspace/${KN}/graph-explorer`;
  const explorer = page;
  await explorer.goto(explorerPath);
  await loginIfNeeded(explorer);
