...
> main 分支暂不放这个按钮，页面只能直接输 URL 进入（§4.2 路由）。以下是 release/0.1.4-yf 上的做法，恢复入口时照此加回。
...
0. 对象类定义里配置的显示属性（`display_key`，通过 Studio 的对象类接口读取；Context Loader 不返回它）
1. `_display`
2. `display_name`
3. `name`
4. `id`
...
- 拖动模式（工具栏「拖动」）：「单点」只动被拖节点（Shift 多选时整组一起动）；「牵连」直接相邻节点跟随一半位移、隔一层跟随五分之一，固定节点不动。空白处拖动平移画布，Shift+拖动为框选；两者互不干扰。
- 撤销（工具栏或 Ctrl/⌘+Z）回退到上一次加入/移除/清空之前，保留 30 步；「执行历史」抽屉记录每次调用的输入、后端原始返回、耗时与成败，展开/路径条目可重跑；「导出图片」下载整图 PNG；Shift+点击 / Shift+框选多选后「移除所选」或 Delete 键删除；「按概念组分组」每个概念组一个 combo 并切换到 combo 布局；左栏可收起。
...
- 默认布局可在部署时配置：Studio 侧 `config.js` 写 `window.__BKN_STUDIO_RUNTIME__ = { graphExplorer: { layout: "chain" } }`，看图页侧 `graph-view.config.js` 写 `window.__BKN_GRAPH_VIEW__ = { layout: "chain" }`。优先级是链接上的 `layout` > 用户自己选过的（探索页存在本地缓存里）> 部署默认 > 力导向；不在布局清单里的值忽略。
- 布局「路径」把每个连通部分单独放一条带：按关系方向从左到右分列（没有入边的节点在第 0 列），同列节点竖排并在带内居中，带与带之间留空。按 ID 取子图常得到若干互不相连的短链，力导向会把它们揉成一团，这个布局让每条路径成为一条直线。
...
## 7.5 独立查看页 `graph-view.html`

给外部客户端（如 OpenClaw 这类自带工具调用的 Agent 前端）一个「拿到 URL 就能看图」的页面，不进 Studio 壳、不过登录门。
...
- 页面侧把「Cypher 执行」与「搜索命中补全」拆成不带回合的 `cypherCore` / `enrichCore`，Cypher Tab 与 AI 探索共用；执行历史记一条 `explore` 条目（输入 = 模型/问题/步数上限，输出 = 总结 + 全部步骤）。
...
1. 直接打开 `/studio/knowledge-network/workspace/<networkId>/graph-explorer`，页面正常加载（main 上没有入口按钮，见 §4.1）。
2. 语义搜索一个已知实例 → 加入画布。
