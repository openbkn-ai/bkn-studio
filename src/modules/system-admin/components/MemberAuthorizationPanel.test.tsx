...
vi.mock("antd", async (importOriginal) => ({
  ...
  Drawer: ({ children, open }: { children: ReactNode; open: boolean }) =>
    open ? <div data-testid="permission-drawer">{children}</div> : null,
}));
...
vi.mock("@/modules/system-admin/components/DirectoryUserPicker", () => ({
  DirectoryUserPicker: ({
    ...
    <div>
      <button
        onClick={() =>
          onUsersChange([{ id: "user-1", account: "user-1", name: "User One", roleIds: [] }])
        }
        type="button"
      >
        select-user
  ...
  AppTable: ({
    ...
    <div data-loading={String(loading)} data-testid="grant-table">
      {dataSource.map((grant, index) => (
        <div key={grant.objId}>{columns[2]?.render?.(undefined, grant, index)}</div>
      ))}
    </div>
  ),
}));
...
describe("MemberAuthorizationPanel", () => {
  ...
  it("shows grants without waiting for slow resource-name enrichment", async () => {
    ...
    expect(screen.getByTestId("grant-table").textContent).toContain(
      "systemAdmin.objectGrants.memberPermissionDetail",
    );
    expect(resolveGrantNamesMock).toHaveBeenCalled();

    fireEvent.click(
      screen.getByRole("button", { name: "systemAdmin.objectGrants.memberPermissionDetail" }),
    );
  ...
  it("renders the super-admin wildcard without requesting object-grant records", async () => {
    ...
    expect(
      screen.getByLabelText("systemAdmin.objectGrants.memberSuperAdminTitle").textContent,
    ).toBe("*");
