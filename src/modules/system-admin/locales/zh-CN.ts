...
export const systemAdminZhCN = {
  systemAdmin: {
    ...
    resourceCatalog: {
      ...
      operationDescriptions: {
        ...
        catalog: {
          ...
          view_summary:
            "查看数据目录摘要以便导航。该操作由 bkn-safe 在鉴权读取时计算，不能配置为允许或禁止规则。",
          create: "创建数据目录。",
          modify:
          ...
          query_data:
            "查询数据目录下资源的数据，支持数据集和物理表。当目录下数据资源未显式授予查询权限时，可回退到数据目录的查询权限。需要同时具有查看数据目录权限。",
          data_write:
        ...
        resource: {
          ...
          modify:
            "修改数据资源配置。未显式授予时，可回退到所属数据目录的资源管理权限。需要同时具有查看数据资源权限。",
          delete:
            "删除数据资源。未显式授予时，可回退到所属数据目录的资源管理权限。需要同时具有查看数据资源权限。",
          query_data:
            "查询数据资源的数据。未显式授予时，可回退到所属数据目录的查询权限。需要同时具有查看数据资源权限。",
          data_write:
            "仅可写入或删除数据集文档；物理表不支持数据写入。未显式授予时，可回退到所属数据目录的写入权限。需要同时具有查看数据资源权限。",
        },
        connector_type: {
    ...
    accessOrigins: {
      ...
      delete: "删除",
      deleteTitle: "删除访问地址",
      deleteConfirm: "确认删除“{{origin}}”吗？删除后，从该地址发起的新登录和退出流程将无法完成。",
      callback: "登录回调",
      logout: "退出返回",
      empty: "暂无可用访问地址",
      columns: {
      ...
      security: {
        https: "HTTPS",
        http: "HTTP（不安全）",
        httpHint: "HTTP 传输不会加密登录流量，仅建议在受信任的内网或本地开发环境使用。",
      },
      readOnly: {
        label: "只读",
        system: "系统主地址由部署配置管理，不能在此删除。",
        deployment: "该地址来自 Helm 部署配置，不能在此删除。",
        runtime: "该地址当前不可删除。",
      },
      validation: {
        invalid: "请输入完整的 HTTP(S) origin，且不要包含路径、用户信息、查询参数、片段或通配符。",
        duplicate: "该访问地址已存在。",
      },
      toast: {
        created: "访问地址已添加并同步",
        savedPending: "访问地址已保存，正在等待认证服务同步",
        deleted: "访问地址已删除",
        deletePending: "删除请求已保存，正在等待认证服务同步",
        synced: "全部访问地址已同步到认证服务",
        syncPending: "同步未完成，认证服务恢复后将自动重试",
      },
      errors: {
        create: "添加访问地址失败：{{error}}",
        delete: "删除访问地址失败：{{error}}",
        notFound: "该访问地址已不存在，列表已刷新。",
        deleteConflict: "当前状态不允许删除该地址，列表已刷新。",
        sync: "立即同步失败：{{error}}",
      },
    },
    audit: {
