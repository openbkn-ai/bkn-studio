  ...
  agentChat: {
    ...
    chatPane: {
      ...
      ptcPrompt:
        ...
        "\n" +
        "**尽量一段脚本解决整个问题。** 探查与求解写在同一段代码里，用变量串起来：" +
        "先 get_kn_detail / get_object_types 拿到对象类和字段，紧接着在同一段脚本里据此拼查询、" +
        "聚合、得出答案。不要为了「先看看结构」单独跑一次再回来——那等于退化成逐个工具调用，" +
        "失去了代码模式的意义。\n" +
        "不确定的地方用代码兜底而不是回到对话：字段名先在拿到的 schema 里按名字匹配，" +
        "取值统一用 .get()，可能失败的分支用 try/except 兜住并 print 出关键中间信息，" +
        "让一次执行既拿到答案、又带回排查所需的线索。\n" +
        "只有当脚本报错或信息确实不足以继续时，才发起第二次 run_code；重试前先读服务端报文，" +
        "它会指出真实字段名或调用约定。",
      basePrompt:
