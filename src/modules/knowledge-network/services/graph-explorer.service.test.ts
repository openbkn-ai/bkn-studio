...
describe("buildInstanceId", () => {
  it("mirrors GetObjectID for a single primary key", () => {
    expect(buildInstanceId("ot_supplier", ["supplier_id"], { supplier_id: 42 })).toBe(
      "ot_supplier-42",
    );
  });
  ...
  it("joins composite keys in primary_keys order and marks missing keys", () => {
    expect(
      buildInstanceId("ot_order", ["order_no", "line_no"], { line_no: 3, order_no: "A1" }),
    ).toBe("ot_order-A1_3");
    expect(buildInstanceId("ot_order", ["order_no", "line_no"], { order_no: "A1" })).toBe(
      "ot_order-A1___NULL__",
    );
  });
...
describe("identityCondition", () => {
  it("emits one leaf for a single key and an and-group for composite keys", () => {
    expect(identityCondition({ supplier_id: 42 })).toEqual({
      field: "supplier_id",
      operation: "==",
      value: 42,
    });
...
describe("fromQueryObjectInstance", () => {
  ...
  it("converges on the same id as a search hit for the same instance", () => {
    const fromQuery = fromQueryObjectInstance(supplier, {
      datas: [{ supplier_id: 42, city: "Shanghai" }],
    })[0];
    const fromSearch = fromSearchInstance(
      {
        nodes: [
          { object_type_id: "ot_supplier", unique_identities: { supplier_id: 42 }, properties: {} },
        ],
      },
...
const subgraphPayload = {
  ...
  isolated_objects: {
    "ot_supplier-43": {
      _instance_id: "ot_supplier-43",
      object_type_id: "ot_supplier",
      object_type_name: "供应商",
      properties: { city: "Wuhan" },
    },
  },
  relation_paths: [
    {
      ...
      relations: [
        {
          relation_type_id: "rt_supplies",
          relation_type_name: "供货",
          source_object_id: "ot_supplier-42",
          target_object_id: "ot_order-A1_3",
        },
        {
          relation_type_id: "rt_contains",
          relation_type_name: "包含",
          source_object_id: "ot_order-A1_3",
          target_object_id: "ot_product-P9",
        },
      ],
    },
    {
...
describe("meetInTheMiddle", () => {
  const rel = (id: string, s: string, t: string) => ({
    relation_type_id: id,
    relation_type_name: id,
    source_object_id: s,
    target_object_id: t,
  });
  it("joins a chain from each side at the first shared node, keeping relation directions", () => {
    ...
    expect(meetInTheMiddle(fromA, "A", fromB, "B")).toEqual([
      rel("r1", "A", "T"),
      rel("r2", "B", "T"),
    ]);
  });
...
describe("effectiveLabelsFrom", () => {
  it("takes the display key per type and lets the user override it", () => {
    const metas = {
      a: { id: "a", name: "a", primaryKeys: ["k"], properties: [], displayKey: "title" },
      b: { id: "b", name: "b", primaryKeys: ["k"], properties: [] },
    };
...
describe("mergeGraph", () => {
  ...
  it("adds only unseen nodes and only edges whose endpoints exist", () => {
    ...
    const first = mergeGraph(nodes, edges, {
      nodes: [node("a"), node("b")],
      edges: [edge("a", "b"), edge("b", "zzz")],
    });
...
describe("parseIdList", () => {
  it("resolves instance ids by the longest object type prefix and falls back to the selected type", () => {
    const parsed = parseIdList(
      ...
      ["product", "product_line", "knowledge"],
      "block",
    );
  ...
  it("reports raw keys as unknown when no object type is selected", () => {
    expect(parseIdList("12345\nproduct-x", ["product"])).toEqual({
      items: [{ otId: "product", key: "x" }],
      unknown: ["12345"],
    });
  });
});
...
describe("keyValueFor / edgesAmong", () => {
  it("types numeric keys and keeps strings", () => {
    expect(
      keyValueFor(
        { id: "a", name: "a", primaryKeys: ["k"], properties: [{ name: "k", type: "integer" }] },
        "42",
      ),
    ).toBe(42);
    expect(
      keyValueFor(
        { id: "a", name: "a", primaryKeys: ["k"], properties: [{ name: "k", type: "string" }] },
        "42",
      ),
    ).toBe("42");
  });
  ...
  it("keeps only edges with both ends in the set", () => {
    ...
    expect(
      edgesAmong([edge("a", "b"), edge("a", "z")], new Set(["a", "b"])).map((e) => e.id),
    ).toEqual(["a|r|b"]);
  });
});
    ...
    expect(
      buildKnSearchBody(
        ...
        "q",
        {
          ...
          conceptGroups: ["cg"],
          maxObjectTypes: 2,
          maxInstancesPerType: 7,
        },
        { ...DEFAULT_RRF_OPTIONS, rrfK: 30, knnWeight: 0.8, rerankMode: "shadow" },
      ),
    ).toEqual({
      query: "q",
      kn_id: "kn1",
...
describe("createGraphExplorerClient", () => {
      ...
      object_types: [
        {
          ...
          name: "A",
          primary_keys: ["k"],
          data_properties: [{ name: "k", type: "string" }],
        },
      ],
    });
    const metas = await createGraphExplorerClient(env, auth).loadObjectTypes(["ot_a"]);
    expect(sent(spy)).toMatchObject({
      url: `${REST}/get_object_types?response_format=json`,
      body: { kn_id: "kn1", ids: ["ot_a"] },
    });
    expect(sent(spy).headers).toMatchObject({
    ...
    expect(metas).toEqual([
      {
        ...
        name: "A",
        primaryKeys: ["k"],
        properties: [{ name: "k", displayName: undefined, type: "string" }],
      },
    ]);
  });
  ...
  it("puts kn_id in the query string of query_instance_subgraph", async () => {
    ...
    const paths = [
      {
        object_types: [{ id: "a" }, { id: "b" }],
        relation_types: [
          { relation_type_id: "r", source_object_type_id: "a", target_object_type_id: "b" },
        ],
      },
    ];
    await createGraphExplorerClient(env, auth).queryInstanceSubgraph(paths);
    expect(sent(spy)).toMatchObject({
      url: `${REST}/query_instance_subgraph?kn_id=kn1&response_format=json`,
      body: { relation_type_paths: paths },
    });
  ...
  it("puts kn_id and ot_id in the query string of query_object_instance and pages only when offset is positive", async () => {
    ...
    expect(sent(spy, 0).url).toBe(
      `${REST}/query_object_instance?kn_id=kn1&ot_id=ot_a&response_format=json`,
    );
    expect(sent(spy, 0).body).toEqual({ limit: 50 });
    expect(sent(spy, 1).body).toEqual({
      limit: 2,
      condition: { field: "k", operation: "in", value: [1, 2] },
      offset: 50,
    });
  });
  ...
  it("posts run_cypher with kn_id and the query in the body and keeps only well-formed rows", async () => {
    const spy = mockFetch({
      columns: [{ name: "a", type: "node" }, { name: 7 }],
      entries: [{ a: 1 }, "junk"],
    });
    const result = await createGraphExplorerClient(env, auth).runCypher("MATCH (a:x) RETURN a");
    expect(sent(spy)).toMatchObject({
      url: `${REST}/run_cypher?response_format=json`,
      body: { kn_id: "kn1", query: "MATCH (a:x) RETURN a" },
    });
    expect(result).toEqual({
      columns: [
        { name: "a", type: "node" },
        { name: "7", type: undefined },
      ],
      entries: [{ a: 1 }],
    });
  });
  ...
  it("never sends bkn_context or a body response_format, so no interaction is opened", async () => {
    ...
    await client.exploreSubgraph({
      sourceOtId: "a",
      condition: { field: "k", operation: "==", value: 1 },
      direction: "backward",
      pathLength: 1,
    });
    await client.searchInstances("q");
  ...
  it("surfaces the error body of a failed call so friendlyError can read it", async () => {
    ...
    const error = await createGraphExplorerClient(env, auth)
      .searchInstances("x")
  ...
  it("names the tool and status when a failed call has no body", async () => {
    mockFetch("", 502);
    await expect(
      createGraphExplorerClient(env, auth).exploreSubgraph({
        sourceOtId: "a",
        condition: { field: "k", operation: "==", value: 1 },
        direction: "forward",
        pathLength: 1,
      }),
    ).rejects.toThrow("explore_subgraph failed (502)");
  });
  ...
  it("retries once with a refreshed token after 401", async () => {
    ...
    await createGraphExplorerClient(env, {
      getToken: () => "old",
...
describe("knSearchInstances", () => {
  ...
  it("posts the fusion body to kn_search without bkn_context", async () => {
    const spy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue(
        new Response(JSON.stringify({ nodes: [], object_types: [] }), { status: 200 }),
      );
    const rrf = { ...DEFAULT_RRF_OPTIONS, rrfK: 30 };
    await knSearchInstances(
      { base: "https://studio.example.com", token: "t", knId: "kn1" },
      undefined,
      "q",
      {},
      rrf,
    ...
    expect(url).toBe(
      "https://studio.example.com/api/agent-retrieval/v1/kn/kn_search?response_format=json",
    );
...
describe("friendlyError", () => {
  it("unwraps the downstream envelope embedded in the Context Loader details", () => {
    const downstream = JSON.stringify({
      error_code: "VegaBackend.Resource.NotFound",
      description: "数据资源不存在",
      solution: "请检查数据资源ID",
      error_details: "",
    });
    ...
    expect(friendlyError(new Error(outer))).toBe(
      "调用依赖服务异常：数据资源不存在（请检查数据资源ID）",
    );
  });
    ...
    expect(
      friendlyError(
        new Error(
          JSON.stringify({
            description: "内部错误",
            details: "QueryDatasetData returned HTTP 500",
          }),
        ),
      ),
    ).toBe("内部错误：QueryDatasetData returned HTTP 500");
    expect(
      friendlyError(
        new Error(JSON.stringify({ error: { message: "conversation_id is required" } })),
      ),
    ).toBe("conversation_id is required");
  });
  ...
  it("retries a failed batch one path at a time so one bad relation type costs only its own edges", async () => {
    ...
    const client = {
      queryInstances: vi.fn(() =>
        Promise.resolve({ datas: [{ id: "1", _instance_identity: { id: "1" } }] }),
      ),
      queryInstanceSubgraph: vi.fn(
        (paths: { relation_types: { relation_type_id: string }[] }[]) => {
    ...
    const result = await collectSubgraphByIds(
      client,
      [
        { otId: "a", key: "1" },
        { otId: "b", key: "1" },
    ...
    expect(calls.slice(1, 6).map((names) => names[0])).toEqual([
      "rel0",
      "rel1",
      "rel2",
      "rel3",
...
describe("pickDisplay", () => {
  it("prefers the tidied text over the raw one", () => {
    expect(pickDisplay({ text: "**原文** 带标记", text_clean: "原文带标记" }, "a-1")).toBe(
      "原文带标记",
    );
  ...
  it("keeps the configured and conventional names ahead of any text", () => {
    ...
    expect(pickDisplay({ topic: "主题", description: "说明", text_clean: "正文" }, "a-1")).toBe(
      "主题",
    );
...
describe("orientEdges", () => {
  const relations = new Map([
    [
      "rel_squads_tournament",
      { id: "rel_squads_tournament", sourceOtId: "squads", targetOtId: "tournaments" },
    ],
    ["rel_person_person", { id: "rel_person_person", sourceOtId: "person", targetOtId: "person" }],
  ]);
  const otOf = (id: string) => id.split("-")[0];
  const edge = (source: string, rel: string, target: string) => ({
  ...
  it("swaps an edge reported against its declared direction and rebuilds its id", () => {
    const [out] = orientEdges(
      [edge("tournaments-1", "rel_squads_tournament", "squads-10")],
      relations,
      otOf,
    );
    expect(out).toMatchObject({
      id: "squads-10|rel_squads_tournament|tournaments-1",
      source: "squads-10",
      target: "tournaments-1",
    });
  });
