...
const serverTools: McpToolDef[] = [
  tool("get_kn_detail", {
    ...
    group: "discovery",
    groupTitle: "Network and Schema",
    order: 120,
  }),
  tool("run_sql", {
    title: "SQL Query",
    group: "query",
    groupTitle: "Instance Queries",
    order: 240,
  }),
  tool("bkn_start_interaction", {
...
const groupsOf = (tools: McpToolDef[]) =>
  buildMcpToolGroups(tools, (item) => toolDisplayOf(item.name, item));

describe("toolDisplayOf", () => {
  it("takes the display name and group straight from tools/list instead of the local table", () => {
    const display = toolDisplayOf(
      "run_sql",
      tool("run_sql", {
        title: "SQL Query",
        group: "query",
  ...
  it("falls back to the local catalog when the server sends no display metadata", () => {
    ...
    expect(display).toMatchObject({
      name: "SQL Data Query",
      groupKey: "data",
      groupLabel: "Data Resources and SQL",
      fromServer: false,
    });
  });
  ...
  it("keeps the lifecycle / execution / skill tools in their own groups when the server sends no metadata", () => {
    ...
    expect(toolDisplayOf("bkn_get_operation")).toMatchObject({
      groupKey: "lifecycle",
      name: "View Operation Status",
    });
    expect(toolDisplayOf("bkn_get_receipt")).toMatchObject({
      groupKey: "lifecycle",
      name: "View Call Receipt",
    });
    expect(toolDisplayOf("list_action_execution")).toMatchObject({
      groupKey: "logic",
      name: "Action Execution History",
    });
    expect(toolDisplayOf("query_metric")).toMatchObject({
      groupKey: "logic",
      name: "Metric Data Query",
    });
    expect(toolDisplayOf("read_skill_file")).toMatchObject({
      groupKey: "skill",
      name: "Read Skill File",
    });
    expect(toolDisplayOf("execute_skill")).toMatchObject({
      groupKey: "skill",
      name: "Execute Skill",
    });
  });
  ...
  it("keeps an unknown bkn_ tool in the lifecycle group instead of the knowledge-network group", () => {
    expect(toolDisplayOf("bkn_archive_interaction")).toMatchObject({
      groupKey: "lifecycle",
      name: "Session Lifecycle Tool",
    });
  ...
  it("falls back to the tool name for a tool no side knows", () => {
    ...
    expect(
      toolDisplayOf(
        ...
        tool("brand_new_capability", { title: "Brand New Capability" }),
      ).name,
    ).toBe("Brand New Capability");
  });
});
...
describe("buildMcpToolGroups", () => {
  ...
  it("keeps tools without display metadata at the tail instead of dropping them", () => {
    ...
    expect(lifecycle?.items.map(({ item }) => item.name)).toEqual([
