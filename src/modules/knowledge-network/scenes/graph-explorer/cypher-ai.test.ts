...
import {
  buildCypherPrompt,
  extractCypherFragment,
  inlineMapsToWhere,
  type CypherPromptTexts,
} from "./cypher-ai";
        ...
        object_types: [
          {
            ...
            name: "知识",
            comment: "抽取的知识",
            data_properties: [{ name: "id", type: "string" }, { name: "title" }],
          },
        ],
        relation_types: [
          {
            id: "knowledge_cites_block",
            name: "引用原文块",
            sourceId: "knowledge",
            targetId: "block",
          },
...
describe("extractCypherFragment", () => {
  it("strips fences, leading prose and a trailing RETURN", () => {
    ...
    ).toBe("MATCH (k:knowledge)-[:knowledge_cites_block]->(b:block)\nWHERE k.title <> ''");
  ...
  it("moves inline property maps into the WHERE clause", () => {
    expect(
      inlineMapsToWhere(
        "MATCH (p:product {name: '问界M7'})<-[:knowledge_about_product]-(k:knowledge)",
      ),
    ).toBe("MATCH (p:product)<-[:knowledge_about_product]-(k:knowledge)\nWHERE p.name = '问界M7'");
    expect(
      inlineMapsToWhere(
        "MATCH (p:product {name: 'a, b', seats: 5})-[:r]->(k:knowledge) WHERE k.title <> ''",
      ),
    ).toBe(
