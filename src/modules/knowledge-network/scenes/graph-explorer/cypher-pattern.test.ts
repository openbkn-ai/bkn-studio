...
describe("parseCypherPattern", () => {
  it("reads variables, labels and directed relationships from a chain", () => {
    const parsed = parseCypherPattern(
      "MATCH (k:knowledge)-[:knowledge_cites_block]->(b:block)<-[r:fact_cites_block]-(f:fact) WHERE k.title <> 'x'",
    );
  ...
  it("prepends MATCH when the user starts with the pattern and accepts Chinese labels", () => {
    const parsed = parseCypherPattern(
      "(k:知识)-[:knowledge_about_component]->(c:零件品类)",
    ) as CypherPattern;
  ...
  it("reads several paths, comma-separated or as more MATCH clauses", () => {
    ...
    const comma = parseCypherPattern(
      "MATCH (t:teams)<-[:r1]-(pa:appearances), (pa)-[:r2]->(p:players)",
    );
  ...
  it("rejects empty input, a RETURN clause, and unlabeled variables", () => {
    ...
    expect(parseCypherPattern("MATCH (k:knowledge) RETURN k.id")).toEqual({
      error: "return_present",
    });
    expect(parseCypherPattern("MATCH (k)-[:r]->(b:block)")).toEqual({
      error: "unlabeled",
      detail: "k",
    });
...
describe("buildCypherQuery / cypherRowsToGraph", () => {
  const pattern = parseCypherPattern(
  ...
  const edges = [
    {
      from: "k",
      to: "b",
      relation: "knowledge_cites_block",
      relTypeId: "knowledge_cites_block",
      relTypeName: "引用原文块",
    },
  ];
  ...
  it("turns rows into deduplicated nodes and edges keyed like the rest of the explorer", () => {
    ...
    expect(graph.edges.map((e) => e.id)).toEqual([
      "knowledge-k1|knowledge_cites_block|block-b1",
      "knowledge-k1|knowledge_cites_block|block-b2",
    ]);
  });
});
