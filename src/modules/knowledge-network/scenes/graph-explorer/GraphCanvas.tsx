  ...
  public execute(model: GraphData): Promise<GraphData> {
    ...
    return Promise.resolve({
      nodes: nodes.map((node, index) => ({
        ...node,
        style: { ...node.style, x: points[index]?.x ?? 0, y: points[index]?.y ?? 0 },
      })),
      edges: model.edges ?? [],
      combos: model.combos ?? [],
    });
...
function degreeStats(graph: Graph): {
  count: number;
...
function layoutOptions(layout: ExplorerLayout, graph: Graph | null): LayoutOptions {
  const stats = graph
    ? degreeStats(graph)
    : { count: 0, degree: new Map<string, number>(), hubId: null, maxDegree: 0 };
  ...
  switch (layout) {
    ...
    default:
      ...
      return {
        type: "d3-force",
        link: {
          distance: (edge: { source: string; target: string }) =>
            clampRadius(
              Math.max(
                stats.degree.get(String(edge.source)) ?? 0,
                stats.degree.get(String(edge.target)) ?? 0,
              ),
            ),
          strength: 0.3,
        },
...
function toNodeData(node: GNode, position?: NodePosition, combo?: string): NodeData {
  const data: NodeData = {
    id: node.id,
    data: { otId: node.otId, otName: node.otName, display: node.display },
  };
...
function toEdgeData(edge: GEdge): EdgeData {
  return {
    ...
    source: edge.source,
    target: edge.target,
    data: { relTypeName: edge.relTypeName },
  };
}
...
export const GraphCanvas = forwardRef<GraphCanvasHandle, GraphCanvasProps>(
  function GraphCanvas(props, ref) {
    ...
    const marksRef = useRef<CanvasMarks>({
      pathStart: null,
      pathEnd: null,
      pinned: new Set(),
    ...
    const activeLayout = (): LayoutOptions =>
      groupingRef.current ? COMBO_LAYOUT : layoutOptions(layoutRef.current, graphRef.current);
    ...
    const readPositions = useCallback((): Record<string, NodePosition> => {
      ...
      for (const node of graph.getNodeData()) {
        ...
        out[String(node.id)] = marksRef.current.pinned.has(String(node.id))
          ? { x, y, fixed: true }
          : { x, y };
      }
      return out;
    ...
    useEffect(() => {
      ...
      if (!container) return;
      const { initialNodes, initialEdges, initialPositions, layout, shape, colorOf, menuLabels } =
        propsRef.current;
      ...
      const graph = new Graph({
        ...
        node: {
          ...
          style: {
            size: 36,
            fill: (d: NodeData) =>
              propsRef.current.colorOf(stringifyValue(d.data?.otId)) || colorOf(""),
            stroke: "#ffffff",
            lineWidth: 1.5,
            labelText: (d: NodeData) =>
              labelsRef.current.node
                ? truncate(stringifyValue(d.data?.display) || String(d.id))
                : "",
        ...
        edge: {
          ...
          style: {
            ...
            labelText: (d: EdgeData) =>
              labelsRef.current.edge ? stringifyValue(d.data?.relTypeName) : "",
            labelFontSize: 10,
            labelFill: "#5c6270",
            labelBackground: true,
        ...
        behaviors: [
          ...
          // it) and never while Shift is held, which is the brush-select gesture.
          {
            ...
            enable: (event: { shiftKey?: boolean; targetType?: string }) =>
              !event.shiftKey && (event.targetType === undefined || event.targetType === "canvas"),
          },
          "zoom-canvas",
        ...
        plugins: [
          {
            ...
            getItems: (event: IElementEvent) => {
              ...
              return (propsRef.current.menuActions ?? MENU_ORDER).map((action) => {
                ...
                if (action === "setPathStart" && marks.pathStart === id)
                  effective = "clearPathStart";
          ...
          },
          {
            ...
            getContent: (_event: IElementEvent, items: unknown[]) => {
              ...
              const display = escapeHtml(
                stringifyValue(datum?.data?.display) || String(datum?.id ?? ""),
              );
              const otName = escapeHtml(stringifyValue(datum?.data?.otName));
              return Promise.resolve(
                `<div style="font-size:12px;line-height:1.5"><b>${display}</b><br/><span style="color:#8c8c8c">${otName}</span></div>`,
              );
            },
          },
        ],
      ...
      graph.on(NodeEvent.DBLCLICK, (event: IElementEvent) =>
        propsRef.current.onNodeDoubleClick?.(String(event.target.id)),
      );
      graph.on(NodeEvent.DRAG_START, (event: IElementEvent) => {
        ...
        const dragged = new Set(
          graph.getElementState(id).includes("selected")
            ? graph
                .getNodeData()
                .filter((n) => graph.getElementState(n.id).includes("selected"))
                .map((n) => String(n.id))
            : [id],
        );
        ...
        const first = graph
          ...
          .map((n) => String(n.id))
          .filter((n) => !dragged.has(n) && !marksRef.current.pinned.has(n));
        for (const n of first) followers.set(n, 0.5);
        for (const n of first) {
          for (const second of graph.getNeighborNodesData(n).map((m) => String(m.id))) {
            if (
              !dragged.has(second) &&
              !followers.has(second) &&
              !marksRef.current.pinned.has(second)
            )
              followers.set(second, 0.2);
          }
        }
      ...
      (): GraphCanvasHandle => ({
        async addElements(nodes, edges, anchorId) {
          ...
          const clear = (x: number, y: number) =>
            taken.every(([tx, ty]) => Math.hypot(tx - x, ty - y) >= MIN_GAP);
          let placed = 0;
          const nodeData = fresh.map((node) => {
            ...
            const angle =
              (2 * Math.PI * offset) / ring.slots + (ringIndex % 2) * (Math.PI / ring.slots);
            placed += 1;
          ...
          if (groupingRef.current) {
            ...
            const needed = groupingRef.current.combos.filter(
              (combo) =>
                !existingCombos.has(combo.id) && nodeData.some((item) => item.combo === combo.id),
            );
            if (needed.length > 0)
              graph.addComboData(
                needed.map((combo) => ({ id: combo.id, data: { name: combo.name } })),
              );
          }
        ...
        setLayout(layout) {
          ...
          graphRef.current?.setLayout(
            groupingRef.current ? COMBO_LAYOUT : layoutOptions(layout, graphRef.current),
          );
        },
        ...
        async replaceAll(nodes, edges, positions) {
          ...
          const usedCombos = new Set(
            nodes.map((node) => comboFor(node.otId)).filter((id): id is string => Boolean(id)),
          );
          graph.setData({
            ...
            combos: grouping
              ? grouping.combos
                  ...
                  .map((combo) => ({ id: combo.id, data: { name: combo.name } }))
              : [],
          });
          await graph.draw();
