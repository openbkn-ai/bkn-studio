  ...
  return (
    <div className={styles.toolbar} data-testid="graph-explorer-toolbar">
      <Tooltip
        title={t(
          sidebarCollapsed
            ? "knowledgeNetwork.graphExplorer.toolbar.expandSidebar"
            : "knowledgeNetwork.graphExplorer.toolbar.collapseSidebar",
        )}
      >
        <Button
          size="small"
          data-testid="graph-explorer-sidebar-toggle"
      ...
      <Space size={8} wrap className={styles.group}>
        ...
        <Select
          ...
          options={LAYOUTS.map((item) => ({
            value: item,
            label: t(`knowledgeNetwork.graphExplorer.layouts.${item}`),
          }))}
        ...
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.relayout")}>
          <Button
            size="small"
            data-testid="graph-explorer-relayout"
            icon={<ReloadOutlined />}
        ...
        <Select
          ...
          options={SHAPES.map((item) => ({
            value: item,
            label: t(`knowledgeNetwork.graphExplorer.shapes.${item}`),
          }))}
          onChange={onShapeChange}
        />
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.dragModeHelp")}>
          <span className={styles.label}>
            {t("knowledgeNetwork.graphExplorer.toolbar.dragMode")}
          </span>
        </Tooltip>
        <Select
          size="small"
        ...
        {canGroup ? (
          <>
            <span className={styles.label}>
              {t("knowledgeNetwork.graphExplorer.toolbar.groupByConceptGroup")}
            </span>
            <Switch
              size="small"
              data-testid="graph-explorer-group"
              checked={groupByConceptGroup}
              disabled={busy}
        ...
        <span className={styles.label}>
          {t("knowledgeNetwork.graphExplorer.toolbar.nodeLabels")}
        </span>
        <Switch
          size="small"
          data-testid="graph-explorer-node-labels"
          checked={showNodeLabels}
          onChange={(checked) => onLabelVisibilityChange(checked, showEdgeLabels)}
        />
        <span className={styles.label}>
          {t("knowledgeNetwork.graphExplorer.toolbar.edgeLabels")}
        </span>
        <Switch
          size="small"
          data-testid="graph-explorer-edge-labels"
          checked={showEdgeLabels}
          onChange={(checked) => onLabelVisibilityChange(showNodeLabels, checked)}
        />
      ...
      <Space size={6} className={styles.group}>
        ...
        >
          {t("knowledgeNetwork.graphExplorer.toolbar.pathStart")}:{" "}
          {pathStart?.display ?? t("knowledgeNetwork.graphExplorer.toolbar.unset")}
        </Tag>
        <Tag
          color="red"
          className={styles.pathTag}
        ...
        >
          {t("knowledgeNetwork.graphExplorer.toolbar.pathEnd")}:{" "}
          {pathEnd?.display ?? t("knowledgeNetwork.graphExplorer.toolbar.unset")}
        </Tag>
        <Button
          size="small"
          type="primary"
      ...
      <Space size={6} className={styles.group}>
        <Typography.Text
          ...
          data-testid="graph-explorer-stats"
        >
          {t("knowledgeNetwork.graphExplorer.toolbar.stats", {
            nodes: nodeCount,
            edges: edgeCount,
          })}
        </Typography.Text>
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.undo")}>
          <Badge count={undoCount} size="small" offset={[-2, 2]} overflowCount={99}>
            <Button
              size="small"
              data-testid="graph-explorer-undo"
              icon={<UndoOutlined />}
        ...
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.export")}>
          <Button
            size="small"
            data-testid="graph-explorer-export"
        ...
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.share")}>
          <Button
            size="small"
            data-testid="graph-explorer-share"
            icon={<LinkOutlined />}
            disabled={nodeCount === 0}
            onClick={onShare}
          />
        ...
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.history")}>
          ...
          >
            <Button
              size="small"
              data-testid="graph-explorer-history"
        ...
        <Tooltip title={t("knowledgeNetwork.graphExplorer.toolbar.removeSelectedHelp")}>
          <Button
            size="small"
            data-testid="graph-explorer-remove-selected"
        ...
        <Popconfirm
          title={t("knowledgeNetwork.graphExplorer.toolbar.clear")}
          onConfirm={onClear}
          disabled={nodeCount === 0}
        >
        ...
        <Popconfirm
          title={t("knowledgeNetwork.graphExplorer.toolbar.clearCache")}
...
type LabelPickerProps = Pick<
  ExplorerToolbarProps,
  "canvasObjectTypes" | "propertyNamesByOt" | "labelByOt" | "onLabelChange"
>;
...
function LabelPickerInner({
  ...
  onLabelChange,
  defaultLabel,
}: LabelPickerProps & { defaultLabel: string }) {
  ...
  useEffect(() => {
    if (!otId || !canvasObjectTypes.some((item) => item.id === otId))
      setOtId(canvasObjectTypes[0]?.id);
