...
import {
  ...
  type McpSchemaField,
} from "@/modules/knowledge-network/services/mcp-schema-doc";
...
function SchemaFieldTable({ fields }: { fields: McpSchemaField[] }) {
  ...
  if (fields.length === 0)
    return (
      <div className={styles.schemaNoFields}>
        {t("knowledgeNetwork.contextLoaderPanel.schema.noFields")}
      </div>
    );
    ...
    <div className={styles.schemaFieldTable} role="table">
      <div className={styles.schemaFieldHeader} role="row">
        ...
        <span role="columnheader">
          {t("knowledgeNetwork.contextLoaderPanel.schema.requirement")}
        </span>
        <span role="columnheader">
          {t("knowledgeNetwork.contextLoaderPanel.schema.description")}
        </span>
      </div>
      {fields.map((field) => (
        <div key={field.path} className={styles.schemaFieldRow} role="row">
          <code
            role="cell"
            className={styles.schemaFieldName}
            style={{ paddingInlineStart: `${field.depth * 16}px` }}
            title={field.path}
          >
            {field.name}
          </code>
          <span role="cell" className={styles.schemaType}>
            {field.type}
          </span>
          <span
            role="cell"
            className={field.required ? styles.schemaRequired : styles.schemaOptional}
          >
            {field.required
              ? t("knowledgeNetwork.contextLoaderPanel.schema.required")
              : t("knowledgeNetwork.contextLoaderPanel.schema.optional")}
          </span>
          <span role="cell" className={styles.schemaDescription}>
            ...
            {field.defaultValue !== undefined ? (
              <em>
                {t("knowledgeNetwork.contextLoaderPanel.schema.defaultValue", {
                  value: formatSchemaValue(field.defaultValue),
                })}
              </em>
            ) : null}
            {field.enumValues ? (
              <em>
                {t("knowledgeNetwork.contextLoaderPanel.schema.enumValues", {
                  values: field.enumValues.map(formatSchemaValue).join(", "),
                })}
              </em>
            ) : null}
            {field.allowsAdditionalProperties ? (
              <em>{t("knowledgeNetwork.contextLoaderPanel.schema.dynamicFields")}</em>
            ) : null}
          </span>
        </div>
      ))}
...
function SchemaExample({ code, onCopy }: { code: string | null; onCopy: () => void }) {
  ...
  if (code === null)
    return (
      <div className={styles.schemaExampleInvalid}>
        {t("knowledgeNetwork.contextLoaderPanel.schema.invalidExample")}
      </div>
    );
  ...
  return (
    <div className={styles.schemaExample}>
      ...
      <pre className={styles.schemaExampleCode}>
        <JsonHighlight text={code} />
      </pre>
    </div>
  );
}
...
export function ContextLoaderIntegrationPanel({
  ...
  const mcpRemoteJsonConfig = createMcpRemoteJsonConfig(
    mcpUrlWithSlash,
    configAppKey,
    mcpClientOptions,
  );
  const claudeCliConfig = createClaudeCodeMcpCommand(
    mcpUrlWithSlash,
    configAppKey,
    mcpClientOptions,
  );
  ...
  const displayOf = useCallback(
    (target: ContextLoaderOp) => {
      ...
      return {
        ...
        groupDescription:
          display.groupDescription === null
            ? null
            : t(`${groupPrefix}.description`, { defaultValue: display.groupDescription }),
        name: t(`knowledgeNetwork.contextLoaderPanel.toolNames.${target.id}`, {
    ...
    return groups
      .map((group) => ({
        ...
        items: group.items.filter(({ item, display }) =>
          `${display.name} ${item.id} ${item.path} ${item.summary} ${group.label}`
            .toLowerCase()
            .includes(filterText),
        ),
      }))
  ...
  const resultContentVisible =
    !isVerifyView || (hasCallState && resultOpen && mcpResultTab === "result");
  const mainClassName = [
    styles.main,
      ...
      <section className={styles.mcpConnectPage}>
        <div className={styles.mcpConnectPanel}>
          <div className={styles.mcpConnectTitleBlock}>
            <h2 className={styles.mcpConnectTitle}>
              {t("knowledgeNetwork.contextLoaderPanel.mcpConnect.title")}
            </h2>
          ...
          <div className={styles.mcpConnectMain}>
            <div className={styles.mcpConfigHeader}>
              ...
              <div className={styles.mcpConfigMeta}>
                ...
                <button
                  type="button"
                  className={styles.mcpConfigureKeyBtn}
                  onClick={openMcpConfigKeyModal}
                >
                  {mcpConfigKey
                ...
                <>
                  <CodeBlock
                    ...
                    onCopy={() =>
                      onCopy(
                        claudeCliConfig,
                        t("knowledgeNetwork.contextLoaderPanel.mcpConnect.copiedClaude"),
                      )
                    }
                  />
                  <CodeBlock
                    ...
                    onCopy={() =>
                      onCopy(
                        mcpRemoteJsonConfig,
                        t("knowledgeNetwork.contextLoaderPanel.mcpConnect.copiedProject"),
                      )
                    }
                  />
                </>
              ...
              {mcpConfigTab === "cursor" ? (
                <CodeBlock
                  ...
                  onCopy={() =>
                    onCopy(
                      mcpRemoteJsonConfig,
                      t("knowledgeNetwork.contextLoaderPanel.mcpConnect.copiedCursor"),
                    )
                  }
                />
              ) : null}
              {mcpConfigTab === "generic" ? (
                <CodeBlock
                  ...
                  onCopy={() =>
                    onCopy(
                      mcpRemoteJsonConfig,
                      t("knowledgeNetwork.contextLoaderPanel.mcpConnect.copiedGeneric"),
                    )
                  }
                />
              ) : null}
        ...
        >
          ...
          />
          {mcpConfigKeyError ? (
            <p className={styles.mcpConfigKeyModalError}>{mcpConfigKeyError}</p>
          ) : null}
          <p className={styles.mcpConfigKeyModalHint}>
            {t("knowledgeNetwork.contextLoaderPanel.appKey.hint")}
          </p>
        </Modal>
      </section>
    ) : null;
      ...
      <div className={mainClassName}>
        <aside className={styles.list}>
          <div className={styles.listHead}>
            <div>
              ...
              <div className={styles.listMeta}>
                {t("knowledgeNetwork.contextLoaderPanel.common.mcpLoaded", { count: 0 })}
              </div>
            </div>
            <button
              type="button"
              className={styles.reloadCapabilitiesBtn}
          ...
          <div className={styles.resEmpty}>
            ...
            <p>
              {toolsError ||
                t("knowledgeNetwork.contextLoaderPanel.empty.noMcpServicesDescription")}
            </p>
          </div>
        </aside>
        <div className={styles.mcpWork}>
          <section className={styles.req}>
            <div className={styles.resEmpty}>
              ...
              <p>
                {t("knowledgeNetwork.contextLoaderPanel.empty.noDebuggableMcpServicesDescription")}
              </p>
              <button
                type="button"
                className={styles.reloadCapabilitiesBtn}
  ...
  const inputSchemaFields = currentTool
    ? splitInputSchemaFields(currentTool.inputSchema)
    : { businessFields: [], traceFields: [], truncated: false };
  const outputSchema = currentTool
    ? schemaDocumentation(currentTool.outputSchema)
        ...
        <>
          <aside className={styles.list}>
            <div className={styles.listHead}>
              <div>
                ...
                <div className={styles.listMeta}>
                  {mode === "mcp"
                    ? t("knowledgeNetwork.contextLoaderPanel.common.mcpLoaded", {
                        count: activeOps.length,
                      })
                    : t("knowledgeNetwork.contextLoaderPanel.common.restAvailable", {
                        count: activeOps.length,
                      })}
                </div>
              </div>
          ...
          <div className={isVerifyView ? styles.mcpWork : styles.mcpWorkInline}>
            <section className={styles.req}>
                  ...
                  {isVerifyView ? (
                    <div className={styles.reqActions}>
                      {mode === "mcp" ? (
                        ...
                        >
                          <FileTextOutlined />{" "}
                          {t("knowledgeNetwork.contextLoaderPanel.request.doc")}
                        </button>
                      ) : null}
                      <button
                        type="button"
                      ...
                      <div className={styles.callParamsHead}>
                        <div className={styles.callParamsTitle}>
                          ...
                          {queryParamsCollapsible && !queryParamsOpen ? (
                            <span className={styles.callParamsCollapsedHint}>
                              {t("knowledgeNetwork.contextLoaderPanel.request.queryCollapsed", {
                                count: visibleQuery.length,
                              })}
                            </span>
                          ) : null}
                        </div>
                        {queryParamsCollapsible ? (
                          ...
                          >
                            ...
                            <DownOutlined
                              className={queryParamsOpen ? styles.callParamsChevronOpen : undefined}
                            />
                          </button>
                        ) : null}
                      </div>
                      ...
                      <div className={styles.qp}>
                        {visibleQuery.map((param) => (
                          <QueryParamRow
                            ...
                            label={
                              mode === "mcp" && param.name === "response_format"
                                ? t("knowledgeNetwork.contextLoaderPanel.request.params")
                                : undefined
                            }
                            emphasized={mode === "mcp" && param.name === "response_format"}
                            value={
                              param.name === "kn_id" ? knId : (queryVals[param.name] ?? param.value)
                            }
                  ...
                  <div className={`${styles.sec} ${isVerifyView ? styles.callParamsSec : ""}`}>
                    {isVerifyView ? (
                      <div className={styles.callParamsHead}>
                        <div className={styles.callParamsTitle}>
                          <div className={styles.secHead}>
                            {t("knowledgeNetwork.contextLoaderPanel.request.body")}{" "}
                            <span className={styles.sub}>application/json</span>
                          </div>
                          {!callParamsOpen ? (
                            <span className={styles.callParamsCollapsedHint}>
                        ...
                        <div className={styles.callParamsTools}>
                          {opSupportsTestData(op.id) ? (
                            <Tooltip
                              title={t(
                                "knowledgeNetwork.contextLoaderPanel.request.autoFillTooltip",
                              )}
                            >
                              <button
                                type="button"
                          ...
                          >
                            ...
                            <DownOutlined
                              className={callParamsOpen ? styles.callParamsChevronOpen : undefined}
                            />
                          </button>
                        </div>
                      </div>
              ...
              {!isVerifyView ? (
                <div className={styles.actions}>
                  ...
                  {opSupportsTestData(op.id) ? (
                    <Tooltip
                      title={t("knowledgeNetwork.contextLoaderPanel.request.autoFillRestTooltip")}
                    >
                      <button
                        type="button"
            ...
            {dataAssistantOpen ? (
              <Modal
                ...
                title={
                  <span className={styles.requestDataAssistantModalTitle}>
                    <DatabaseOutlined />{" "}
                    {t("knowledgeNetwork.contextLoaderPanel.dataAssistant.title")}
                  </span>
                }
                footer={null}
                width={980}
              ...
              >
                <div className={styles.requestDataAssistantModalIntro}>
                  <div>
                    <strong>
                      {t("knowledgeNetwork.contextLoaderPanel.dataAssistant.modalTitle", {
                        name: opDisplay.name,
                      })}
                    </strong>
            ...
            {mode === "mcp" ? (
              <Modal
                open={schemaOpen}
                title={t("knowledgeNetwork.contextLoaderPanel.schema.title", {
                  name: opDisplay.name,
                })}
                footer={null}
                width={920}
              ...
              >
                ...
                {toolsLoading ? (
                  ...
                  <div className={styles.schemaHint}>
                    {t("knowledgeNetwork.contextLoaderPanel.schema.loadFailed", {
                      error: toolsError,
                    })}
                  </div>
                ) : currentTool ? (
                  <>
                    <div
                      ...
                      role="tablist"
                      aria-label={t("knowledgeNetwork.contextLoaderPanel.schema.view")}
                    >
                      <button
                        type="button"
                        role="tab"
                        aria-selected={schemaTab === "docs"}
                        className={schemaTab === "docs" ? styles.schemaTabOn : styles.schemaTab}
                        onClick={() => setSchemaTab("docs")}
                      >
                      ...
                      <button
                        type="button"
                        role="tab"
                        aria-selected={schemaTab === "raw"}
                        className={schemaTab === "raw" ? styles.schemaTabOn : styles.schemaTab}
                        onClick={() => setSchemaTab("raw")}
                      >
                      ...
                      <div className={styles.schemaDocs}>
                        <section className={styles.schemaOverview}>
                          ...
                          <p>
                            {schemaOverviewExpanded
                              ? schemaOverview
                              : toolSummaryPreview(schemaOverview)}
                          </p>
                          {isLongToolSummary(schemaOverview) ? (
                            <button
                              type="button"
                              className={styles.schemaOverviewToggle}
                              onClick={() => setSchemaOverviewExpanded((open) => !open)}
                            >
                              {schemaOverviewExpanded
                        ...
                        <SchemaExample
                          ...
                          onCopy={() => {
                            if (schemaExample !== null)
                              onCopy(
                                schemaExample,
                                t("knowledgeNetwork.contextLoaderPanel.schema.copiedExample"),
                              );
                          }}
                        />
                        <section className={styles.schemaDocSection}>
                          <div className={styles.schemaDocSectionHead}>
                            <div>
                              <h3>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.businessParams")}
                              </h3>
                              <p>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.businessParamsHint")}
                              </p>
                            </div>
                          </div>
                          <SchemaFieldTable fields={inputSchemaFields.businessFields} />
                          {inputSchemaFields.truncated ? (
                            <div className={styles.schemaTruncated}>
                              {t("knowledgeNetwork.contextLoaderPanel.schema.truncatedFields")}
                            </div>
                          ) : null}
                        </section>
                        {inputSchemaFields.traceFields.length > 0 ? (
                          <details className={styles.schemaTrace}>
                            <summary>
                              <span>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.traceTitle")}
                              </span>
                              <span>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.traceBadge")}
                              </span>
                            </summary>
                        ...
                        <section className={styles.schemaDocSection}>
                          <div className={styles.schemaDocSectionHead}>
                            <div>
                              <h3>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.responseShape")}
                              </h3>
                              <p>
                                {t("knowledgeNetwork.contextLoaderPanel.schema.responseShapeHint")}
                              </p>
                            </div>
                          </div>
                          {currentTool.outputSchema !== undefined ? (
                            ...
                            <div className={styles.schemaHint}>
                              {t("knowledgeNetwork.contextLoaderPanel.schema.noOutput")}
                            </div>
                          )}
                        </section>
                      </div>
                        ...
                        <CodeBlock
                          ...
                          onCopy={() =>
                            onCopy(
                              ...
                              t("knowledgeNetwork.contextLoaderPanel.schema.copiedInput"),
                            )
                          }
                        />
                        {currentTool.outputSchema !== undefined ? (
                          <CodeBlock
                            ...
                            onCopy={() =>
                              onCopy(
                                ...
                                t("knowledgeNetwork.contextLoaderPanel.schema.copiedOutput"),
                              )
                            }
                          />
                        ) : (
                  ...
                  <div className={styles.schemaHint}>
                    {t("knowledgeNetwork.contextLoaderPanel.schema.missingTool", { id: op.id })}
                  </div>
                ) : (
                  <div className={styles.schemaHint}>
                    {t("knowledgeNetwork.contextLoaderPanel.schema.notLoaded")}
                  </div>
                )}
              </Modal>
            ) : null}
            ...
            <section
              className={`${styles.res} ${isVerifyView && !hasCallState ? styles.resIdle : ""}`}
            >
              ...
              <div
                className={`${styles.rightView} ${!isVerifyView && rightTab !== "res" ? styles.rightHidden : ""} ${isVerifyView && mcpResultTab !== "result" ? styles.rightHidden : ""}`}
              >
                <div className={styles.resHead}>
                  ...
                  {response ? (
                    <>
                      <span
                        className={`${styles.pill} ${response.ok ? styles.pillOk : styles.pillErr}`}
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            onCopy(
                              ...
                              t("knowledgeNetwork.contextLoaderPanel.response.copiedResponse"),
                            )
                          }
                        >
                          <CopyOutlined />{" "}
                          {t("knowledgeNetwork.contextLoaderPanel.response.copyResult")}
                        </button>
                        {isVerifyView ? (
                          <button
                            type="button"
                          ...
                          >
                            {resultOpen
                              ...
                              : t("knowledgeNetwork.contextLoaderPanel.common.expand")}
                            <DownOutlined
                              className={resultOpen ? styles.resultToggleOpen : undefined}
                            />
                          </button>
                        ) : null}
                      </div>
                  ...
                  <div className={styles.resBody}>
                    {sending ? (
                      ...
                      <div className={styles.resError}>
                        ...
                        <div>
                          <strong>
                            {t("knowledgeNetwork.contextLoaderPanel.response.failed")}
                          </strong>
                          <p>{reqError}</p>
                        </div>
                      </div>
                ...
                {!isVerifyView && resultContentVisible ? (
              ...
              {isVerifyView ? (
                <div
                  className={`${styles.rightView} ${mcpResultTab === "debug" ? "" : styles.rightHidden}`}
                >
                  <div className={styles.debugHead}>
                    <div>
                      ...
                      <span className={styles.debugHint}>
                        {t("knowledgeNetwork.contextLoaderPanel.response.debugHint")}
                      </span>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      <CopyOutlined />{" "}
                      {t("knowledgeNetwork.contextLoaderPanel.response.copyCommand")}
                    </button>
                  </div>
