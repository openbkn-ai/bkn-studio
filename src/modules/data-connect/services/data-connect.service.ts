...
const mockConnectorTypes: MockConnectorType[] = [
  ...
  },
  {
    ...
    required_edition: "professional",
    field_config: {
      host: mockField("Host", "Oracle database server host address", "string", true),
      port: mockField("Port", "Oracle listener port", "integer", true),
      username: mockField("Username", "Oracle login username", "string", true),
      password: mockField("Password", "Oracle login password", "string", true, true),
      service_name: mockField("Service name", "Oracle database service name", "string", true),
      schemas: mockField(
        "Schema list",
        "Optional. Leave empty to scan all accessible non-system schemas.",
        "array",
      ...
      options: mockField(
        "Connection options",
        "Optional Oracle driver options such as ssl, timeout, and charset.",
        "object",
        false,
      ),
    },
