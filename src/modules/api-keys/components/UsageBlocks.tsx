...
/** 把密钥（或 <YOUR_API_KEY> 占位）填进 MCP、CLI 和 SDK 用法示例。 */
export function UsageBlocks({ keyValue }: { keyValue: string }) {
  ...
  const cli = buildCliSnippet(keyValue);
  const sdk = buildSdkSnippet(keyValue);
  ...
  return (
    <Tabs
      ...
      items={[
        ...
        {
          key: "cli",
          label: t("apiKeys.usage.tabCli"),
          children: (
            <div className={styles.stack}>
              <CodeBlock title={t("apiKeys.usage.cli")} code={cli} onCopy={() => copy(cli)} />
            </div>
          ),
        },
        {
          key: "sdk",
          label: t("apiKeys.usage.tabSdk"),
          children: (
            <div className={styles.stack}>
              <CodeBlock title={t("apiKeys.usage.sdk")} code={sdk} onCopy={() => copy(sdk)} />
            </div>
          ),
        },
        {
