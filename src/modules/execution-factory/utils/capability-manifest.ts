...
/**
 * Whether the manifest contains real input/output facts to present.
 *
 * Do not use `tool.ioSpec` existence as the criterion. Missing backend metadata rows return an
 * `api_spec: {}` shell (DefaultMetadataInfo), and deserialization failure for a database api_spec
 * produces the same shell. Both parse to a non-undefined empty ioSpec. Rendering from its existence
 * would display missing metadata as zero inputs and outputs, which invents those zeros.
 *
 * Trade-off: genuine zero-parameter tools also omit the tag. The frontend cannot distinguish them from missing metadata, so understate rather than invent.
 */
