...
describe("API creation next-step navigation", () => {
  it.each([
    [
      "quick-api",
      "create API",
      "executionFactory.createdNextStepsViewToolset",
      "/execution-factory/toolboxes/box-created/tools",
    ],
    [
      "quick-api",
      "create API",
      "executionFactory.createdNextStepsEditTool",
      "/execution-factory/toolboxes/box-created/tools/tool-created/edit",
    ],
    [
      "quick-api",
      "create API",
      "executionFactory.createdNextStepsDebug",
      "/execution-factory/toolboxes/box-created/tools/tool-created/edit?focus=debug",
    ],
    [
      "import-openapi",
      "import API",
    ...
    "refreshes grants before the %s next step %s",
    async (mode, createLabel, nextLabel, destination) => {
      let resolveRefresh!: (user: typeof mocks.runtimeConfig.currentUser) => void;
      mocks.refreshCurrentUser.mockImplementation(
        () =>
          new Promise((resolve) => {
      ...
      await act(async () => {
        resolveRefresh({
          permissions: ["execution-factory:toolbox:view", "execution-factory:tool:view"],
        });
        await Promise.resolve();
      });
      expect(mocks.runtimeConfig.currentUser.permissions).toContain(
  ...
  it("reloads the API target if owner permissions cannot be refreshed", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "executionFactory.createdNextStepsViewToolset" }),
    );
