...
vi.mock("@/modules/bkn-trace/evidence-chain/CurrentExplanationPanel", () => ({
  CurrentExplanationPanel: ({ interactionId }: { interactionId: string }) => (
    <div>saved-evidence:{interactionId}</div>
  ),
}));
...
describe("BusinessProvenanceScene", { timeout: 30_000 }, () => {
  ...
  it("opens the exact conversation supplied by an associated log", async () => {
    window.history.replaceState(
      {},
      "",
      "/observability/business-provenance?conversation_id=conv-linked",
    );
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-linked", questionPreview: "关联会话", interactionCount: 1 },
      ],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-linked", questionPreview: "关联轮次" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-linked",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [],
  ...
  it("accepts the Studio camel-case conversation and interaction links", async () => {
    window.history.replaceState(
      {},
      "",
      "/observability/business-provenance?conversationId=conv-linked&interactionId=int-two",
    );
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-linked", questionPreview: "关联会话", interactionCount: 2 },
      ],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [
        { interactionId: "int-one", questionPreview: "问题甲" },
        { interactionId: "int-two", questionPreview: "问题乙" },
      ],
      total: 2,
    });
    getInteraction.mockImplementation((id: string) =>
      Promise.resolve({
        interactionId: id,
        conversationContext: [],
        derivedFacts: [],
  ...
  it("keeps only timeline and evidence views, with execution inside evidence", async () => {
    ...
    getInteraction.mockClear();
    getMarkdown.mockClear();
    getAnalysisHistory.mockClear();
  ...
  it("shows the current input and output above the deterministic timeline", async () => {
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-source", questionPreview: "查询世界杯", interactionCount: 1 },
      ],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [
        {
          interactionId: "int-source",
          roundNumber: 1,
          questionPreview: "查询世界杯",
          resultPreview: "法国夺冠",
        },
      ],
      total: 1,
    ...
    getInteraction.mockResolvedValue({
      interactionId: "int-source",
      interactionQuestion: "查询 **2018 世界杯**",
      interactionResult: "## 结论\n\n法国夺冠",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [
        {
          ...
          input: { mode: "inline", inline: { tournament: "WC-2018" } },
          output: { mode: "inline", inline: { winner: "France" } },
          query: { conditions: { tournament: "WC-2018" }, resultCount: 1 },
          elements: [{ kind: "object", id: "tournament", name: "赛事" }],
          missingFacts: [],
        },
      ],
    });
    ...
    expect(await screen.findByRole("button", { name: "查看完整本轮输入" })).toHaveTextContent(
      "查询 **2018 世界杯**",
    );
    ...
    expect(screen.getByRole("textbox", { name: "知识网络优化事实 Markdown" })).toHaveValue(
      "# 当前交互轮次知识网络优化分析输入\n\n- Operation：`op-1`",
    );
    expect(screen.getByRole("textbox", { name: "知识网络优化事实 Markdown" })).toHaveAttribute(
      "readonly",
    );
    ...
    expect(
      screen.getAllByRole("heading", { name: "查询赛事（query_object_instance）" }).length,
    ).toBeGreaterThan(0);
  ...
  it("collapses and expands the interaction rail without moving it above the workspace", async () => {
    getConversations.mockResolvedValue({
      entries: [{ conversationId: "conv-rail", questionPreview: "轮次导航", interactionCount: 1 }],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-rail", roundNumber: 1, questionPreview: "第一轮问题" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-rail",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [],
  ...
  it("filters the deterministic timeline by recorded call status", async () => {
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-filter", questionPreview: "检查调用", interactionCount: 1 },
      ],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-filter", roundNumber: 1, questionPreview: "检查调用" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-filter",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [
        {
          ...
          toolName: "query_object_instance",
          callStatus: "completed",
          elements: [{ kind: "object", id: "event", name: "赛事" }],
          missingFacts: [],
        },
        {
          ...
          toolName: "run_cypher",
          callStatus: "failed",
          error: { mode: "inline", inline: { message: "timeout" } },
          elements: [{ kind: "relation", id: "uses", name: "使用关系" }],
          missingFacts: [],
        },
      ],
    });
  ...
  it("does not invent a semantic round number when the API omits it", async () => {
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-legacy", questionPreview: "历史会话", interactionCount: 3 },
      ],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-legacy", questionPreview: "历史轮次" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-legacy",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [],
  ...
  it("submits supported conversation filters only when the user queries", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("搜索问题、结果或会话 ID"), {
      target: { value: "采购" },
    });
  ...
  it("discards a late interaction list from the previously selected conversation", async () => {
    let resolveFirst!: (value: {
      entries: Array<{ interactionId: string; questionPreview: string }>;
      total: number;
    }) => void;
    ...
    getInteractions.mockImplementation(({ conversationId }: { conversationId: string }) =>
      conversationId === "conv-a" ? first : second,
    );
    ...
    resolveFirst({
      entries: [{ interactionId: "int-a", questionPreview: "A 的迟到轮次" }],
      total: 1,
    });
    await act(async () => {
      await first;
  ...
  it("shows that interaction rounds are loading before the list is available", async () => {
    let resolveInteractions!: (value: {
      entries: Array<{ interactionId: string; questionPreview: string }>;
      total: number;
    }) => void;
    const pendingInteractions = new Promise<{
      entries: Array<{ interactionId: string; questionPreview: string }>;
      total: number;
    }>((resolve) => {
    ...
    getConversations.mockResolvedValue({
      entries: [
        { conversationId: "conv-loading", questionPreview: "加载中的会话", interactionCount: 1 },
      ],
      total: 1,
    });
    ...
    resolveInteractions({
      entries: [{ interactionId: "int-loading", questionPreview: "已加载轮次" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-loading",
      conversationContext: [],
      derivedFacts: [],
      contextRelations: [],
      operations: [],
  ...
  it("keeps an eight-column conversation list before opening one interaction workspace", async () => {
    ...
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-1", questionPreview: "查询采购订单" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      interactionId: "int-1",
      conversationContext: [
        {
          ...
          sourceInteractionId: "int-prior",
          sourceOperationId: "op-prior",
        },
      ],
      derivedFacts: [
        {
          rule: "changed_query_still_zero_result",
          sourceOperationId: "op-0",
          operationId: "op-1",
          elementId: "purchase_order",
        },
      ],
      contextRelations: [],
      operations: [
        {
          ...
          toolName: "run_sql",
          knowledgeNetworkId: "supplychain_hd0202",
          startedAt: "2026-08-10T19:02:58Z",
          durationMs: 725,
          callStatus: "completed",
          elements: [
            { kind: "object", id: "purchase_order", name: "物料请购单" },
            {
              kind: "property",
              id: "supplier_number",
              name: "供应商编码",
              parentId: "purchase_order",
              field: "supplier_id",
            },
          ],
          query: {
            resourceIds: ["inventory_resource"],
            conditions: { material_number: "101-000015" },
            resultCount: 0,
          },
          missingFacts: [],
        },
  ...
  it("keeps the full conversation question available when the table clamps it", async () => {
    ...
    getConversations.mockResolvedValue({
      entries: [{ conversationId: "conv-clamped", questionPreview: question, interactionCount: 1 }],
      total: 1,
    });
  ...
  it("shows recorded MCP payloads as concise business facts instead of generic detail placeholders", async () => {
    ...
    getInteractions.mockResolvedValue({
      entries: [
        { interactionId: "int-payload", roundNumber: 1, questionPreview: "反查物料相关产品" },
      ],
      total: 1,
    });
    getInteraction.mockResolvedValue({
      ...
      operations: [
        {
          ...
          input: {
            mode: "inline",
            inline: { code: "result = get_kn_detail(kn_id='supply_ontology_hand')" },
          },
        ...
        },
        {
          ...
          output: {
            ...
            inline: {
              structuredContent: {
                object_types: [{ id: "material" }, { id: "product" }],
                relation_types: [{ id: "used_by" }],
              },
            },
          },
          elements: [],
        ...
        },
        {
          ...
          output: {
            ...
            inline: {
              structuredContent: {
                columns: [{ name: "material_id" }, { name: "product_id" }, { name: "quantity" }],
              },
            },
          },
          elements: [],
        ...
        },
        {
          ...
          input: {
            mode: "inline",
            inline: {
              sql: "SELECT product_id FROM {{.inventory_resource}} WHERE material_id = 'M-100'",
            },
          },
          output: {
            mode: "inline",
            inline: {
              content: [
                {
                  type: "text",
                  text: '{"columns":["product_id"],"data":[{"product_id":"P-1"},{"product_id":"P-2"}]}',
                },
              ],
            },
          },
        ...
        },
        {
          ...
          output: {
            mode: "inline",
            inline: { object_types: [{ id: "material" }, { id: "product" }] },
          },
          elements: [],
          missingFacts: [],
        },
    ...
    expect(
      (await screen.findAllByRole("heading", { name: "运行代码（run_code）" })).length,
    ).toBeGreaterThan(0);
    ...
    expect(
      screen.getByRole("heading", { name: "数据资源详情（describe_resource）" }),
    ).not.toBeNull();
    ...
    expect(
      screen.getByRole("heading", { name: "查询物料（query_object_instance）" }),
    ).not.toBeNull();
    expect(
      screen.getAllByText(
        "material_code = 165-001335；或（warehouse = 上海仓；warehouse = 苏州仓）",
      ).length,
    ).toBeGreaterThan(0);
  ...
  it("keeps the selected call inspector beside the 34th call in a self-scrolling workspace", async () => {
    getConversations.mockResolvedValue({
      entries: [{ conversationId: "conv-long", questionPreview: "长时间链", interactionCount: 1 }],
      total: 1,
    });
    getInteractions.mockResolvedValue({
      entries: [{ interactionId: "int-long", roundNumber: 1, questionPreview: "长时间链" }],
      total: 1,
    });
    getInteraction.mockResolvedValue({
