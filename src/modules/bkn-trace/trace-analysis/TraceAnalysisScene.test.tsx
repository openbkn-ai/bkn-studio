...
describe("TraceAnalysisScene", { timeout: 30_000 }, () => {
      ...
      operations: [
        {
          ...
          fact: {
            ...
            input: {
              byteLength: 320,
              inline: {
                resource_id: "d9qpqpco8jec73ec20tg",
                sql: "SELECT parent_material_code FROM t",
              },
  ...
  it("uses the standard level-three page title", async () => {
    ...
    expect(
      await screen.findByRole("heading", { level: 3, name: "bknTrace.traceAnalysis.title" }),
    ).not.toBeNull();
  });
  ...
  it("opens a real technical Trace into summary, execution chain and raw detail", async () => {
    ...
    expect(
      await screen.findByText("分析对比900-000044和900-000063两款智能球阀的BOM物料差异。"),
    ).not.toBeNull();
    ...
    expect(
      span.compareDocumentPosition(operationButton) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).not.toBe(0);
    ...
    expect(writeTextMock).toHaveBeenCalledWith(
      expect.stringContaining("SELECT parent_material_code FROM t"),
    );
  ...
  it("opens an exact trace from a related-log deep link", async () => {
    window.history.replaceState(
      {},
      "",
      "/studio/observability/traces?trace_id=c2c97a9e8afd1259218a9d975ad63e91",
    );
  ...
  it("keeps the latest list response when an earlier page request finishes late", async () => {
    ...
    const pageTwo = new Promise<Awaited<ReturnType<typeof listTechnicalTraces>>>((resolve) => {
      resolvePageTwo = resolve;
    });
    vi.mocked(listTechnicalTraces)
      .mockResolvedValueOnce({
        entries: [{ traceId: "initial", status: "completed", spanCount: 0 }],
        total: 40,
        page: 1,
        pageSize: 20,
        partial: false,
      ...
      .mockResolvedValueOnce({
        entries: [{ traceId: "filtered", status: "completed", spanCount: 0 }],
        total: 1,
        page: 1,
        pageSize: 20,
        partial: false,
    ...
    fireEvent.change(screen.getByPlaceholderText("bknTrace.traceWorkspace.filters.traceId"), {
    ...
    resolvePageTwo({
      entries: [{ traceId: "stale-page-two", status: "completed", spanCount: 0 }],
      total: 40,
      page: 2,
      pageSize: 20,
      partial: false,
  ...
  it("does not reuse a referenced payload after selecting another operation", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "bknTrace.traceWorkspace.loadReferencedPayload" }),
    );
