...
function parse(value: unknown, id: string): CurrentExplanation {
  ...
  if (view.selectedPairGraphs !== undefined) {
    ...
    for (const graph of Object.values(view.selectedPairGraphs)) {
      ...
      if (graph.businessFunctions.some(item => !item || ![item.id, item.displayName, item.capabilityKind, item.businessPurpose, item.logicSummary, item.validationStatus].every(value => typeof value === "string") || !Array.isArray(item.businessInputs) || !Array.isArray(item.businessOutputs) || !Array.isArray(item.operationIds) || !Array.isArray(item.supportsClaimIds) || !Array.isArray(item.schemaRefs) || !item.technicalExecution || !Array.isArray(item.technicalExecution.interfaceNames))) throw new Error("invalid business function");
