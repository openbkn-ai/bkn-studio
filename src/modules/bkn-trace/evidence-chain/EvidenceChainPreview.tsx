...
export function EvidenceChainPreview() {
  ...
  return (
    <main>
      <p>{t("bknTrace.evidenceChain.preview")}</p>
      <input
        aria-label={t("bknTrace.evidenceChain.chooseFile")}
        type="file"
          ...
          void file
            ...
            .then((text) => {
              ...
              if (
                ...
                throw new Error("invalid read model");
              const candidate = input as EvidenceChainView;
              const optionalText = (value: unknown) =>
                value === undefined || typeof value === "string";
              if (
                !optionalText(candidate.question) ||
                !optionalText(candidate.answer) ||
                !optionalText(candidate.revisionLabel) ||
                typeof candidate.status !== "string" ||
                (candidate.notices &&
                  (!Array.isArray(candidate.notices) ||
                    !candidate.notices.every((n) => typeof n === "string")))
              )
              ...
              for (const graph of [candidate.execution, candidate.evidence]) {
                ...
                for (const node of graph.nodes)
                  if (
                    ...
                    !optionalText(node.value) ||
                    !optionalText(node.detail) ||
                    !optionalText(node.technical)
                  )
                    throw new Error("invalid node");
                for (const edge of graph.edges)
                  if (
                    !edge ||
                    ![edge.id, edge.source, edge.target, edge.label].every(
                      (v) => typeof v === "string",
                    )
                  )
              ...
              for (const claim of candidate.claims) {
                if (
                  ...
                  !optionalText(claim.value) ||
                  !optionalText(claim.detail) ||
      ...
      />
      {error && <p role="alert">{t("bknTrace.evidenceChain.invalidFile")}</p>}
      {view && <EvidenceChainPanels view={view} />}
    </main>
  );
}
