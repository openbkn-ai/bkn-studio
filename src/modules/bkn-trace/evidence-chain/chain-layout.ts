...
function column(node: ChainNode, mode: "evidence" | "execution"): number {
  ...
  if (
    ...
    return 3;
  if (
    ["object", "calculation", "function", "query", "api"].includes(node.kind) ||
    node.role === "context" ||
    node.role === "process"
  )
    return 2;
...
export function layoutChain(graph: ChainGraph, mode: "evidence" | "execution") {
  ...
  const names =
    mode === "execution"
      ...
      : ["conclusion", "facts", "context", "sources"];
  let height = 260;
  slots.forEach((slot, index) => {
    ...
    nodes.forEach((node, row) =>
      positions.set(node.id, { x: 28 + index * 310, y: 62 + row * 194, width: 230, height: 154 }),
    );
    height = Math.max(height, 100 + nodes.length * 194);
  });
  return {
    positions,
    width: Math.max(340, slots.length * 310 + 10),
    height,
    columns: slots.map((s) => names[s]),
  };
}
