...
function preferredExecutionProcess(evidence: ChainGraph, execution: ChainGraph, claim?: ChainClaim) {
  if (!claim) return undefined;
  const recorded = upstream(evidence, claim.nodeIds);
  const referencedProcessIds = new Set(recorded.nodes.map(node => node.executionNodeId).filter((id): id is string => Boolean(id)));
  const executionProcessIds = new Set(execution.nodes.filter(node => node.role === "process").map(node => node.id));
  const processForOutputs = (outputs: ChainNode[]) => {
    for (const output of outputs) {
      const processId = execution.edges.find(edge => edge.target === output.id && referencedProcessIds.has(edge.source) && executionProcessIds.has(edge.source))?.source;
      if (processId) return processId;
    }
    return undefined;
  };
  const byLabel = processForOutputs(execution.nodes.filter(node => node.role === "output" && node.label === claim.label));
  if (byLabel) return byLabel;
  const matchingValues = execution.nodes.filter(node => node.role === "output" && node.value !== undefined && claim.value !== undefined && String(node.value) === String(claim.value));
  ...
  while (frontier.length) {
    ...
    const next = evidence.edges.filter(edge => frontier.includes(edge.target) && !seen.has(edge.source)).map(edge => edge.source);
...
function Graph({ graph, title, mode, onSelect, onEdge, selectedId }: { graph: ChainGraph; title: string; mode: "evidence" | "execution"; onSelect: (node: ChainNode) => void; onEdge: (edge: ChainEdge) => void; selectedId?: string }) {
  ...
  const fit = () => {
    ...
    const next = box && box.clientWidth > 0 && box.clientHeight > 0
      ? clampGraphScale(Math.min(1, (box.clientWidth - 32) / width, (box.clientHeight - 32) / height))
      : 1;
    setScale(next);
    setPan({ x: 0, y: 0 });
  };
  useEffect(() => {
    const box = viewport.current;
    const next = box && box.clientWidth > 0 && box.clientHeight > 0
      ? readableGraphScale(Math.min(1, (box.clientWidth - 32) / width, (box.clientHeight - 32) / height))
      : 1;
    setScale(next);
    setPan({ x: 0, y: 0 });
  ...
  const movePan = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    setPan({ x: drag.current.left + event.clientX - drag.current.x, y: drag.current.top + event.clientY - drag.current.y });
  };
  ...
  return <section aria-label={title} className={styles.graph}>
    <header className={styles.graphTools} role="group" aria-label={t("bknTrace.evidenceChain.graphTools")}>
      ...
      <button type="button" onClick={() => zoom(-0.1)} aria-label={t("bknTrace.evidenceChain.zoomOut")}>−</button>
      <button type="button" onClick={() => zoom(0.1)} aria-label={t("bknTrace.evidenceChain.zoomIn")}>＋</button>
    ...
    <div className={styles.canvas} style={{ width, height, transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})` }}>
      ...
      <svg width={width} height={height} className={styles.lines} aria-label={t("bknTrace.evidenceChain.relations")}>
        ...
        {graph.edges.map(edge => {
          ...
          const selectEdge = () => { setActiveEdgeId(edge.id); setActiveNodeId(undefined); onEdge(edge); };
          return <g key={edge.id} data-metadata={metadata} data-selected={activeEdgeId === edge.id}><path data-testid="chain-edge" d={d} markerEnd={`url(#${marker})`} /><path className={styles.edgeHit} d={d} role="button" tabIndex={0} aria-label={edge.label} onClick={selectEdge} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectEdge(); } }} />{!metadata && <text onClick={selectEdge} x={same ? x1 + 22 : (x1 + x2) / 2} y={(y1 + y2) / 2 - 9} textAnchor="middle">{edge.label}</text>}</g>;
        })}
      </svg>
      {graph.nodes.map(node => <button key={node.id} data-kind={node.kind} aria-pressed={(selectedId ?? activeNodeId) === node.id} className={styles.node} style={positions.get(node.id) ? { left: positions.get(node.id)!.x, top: positions.get(node.id)!.y } : undefined} onClick={() => { setActiveNodeId(node.id); setActiveEdgeId(undefined); onSelect(node); }}>
...
function InspectorDialog({ label, onClose, children }: { label: string; onClose: () => void; children: ReactNode }) {
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [onClose]);
  return <div className={styles.inspectorBackdrop} onClick={onClose}>
    <aside className={styles.inspector} role="dialog" aria-modal="true" aria-label={label} onClick={event => event.stopPropagation()}>{children}</aside>
  </div>;
}

interface BusinessFlow { process: ChainNode; inputs: ChainNode[]; outputs: ChainNode[]; parents: ChainNode[] }
...
function businessFlows(graph: ChainGraph): BusinessFlow[] {
  return graph.nodes.filter(node => node.role === "process").flatMap(process => {
    ...
    const parentIds = graph.edges.filter(edge => edge.target === process.id).map(edge => edge.source);
    ...
    const parents = graph.nodes.filter(node => parentIds.includes(node.id) && node.role === "process");
...
function ClaimStates({ claim }: { claim: ChainClaim }) {
  const { t } = useTranslation();
  if (!claim.supportStatus && !claim.attributionStatus) return <em>{t(`bknTrace.evidenceChain.status.${claim.status}`)}</em>;
  return <span className={styles.claimStates}>
    {claim.supportStatus && <em data-state={claim.supportStatus}>{t(`bknTrace.evidenceChain.support.${claim.supportStatus}`)}</em>}
    {claim.attributionStatus && <em>{t(`bknTrace.evidenceChain.attribution.${claim.attributionStatus}`)}</em>}
  </span>;
}

function ExecutionFlows({ graph, preferredProcessId, preferredClaimId, selectedId, relatedClaimsByProcess, onSelect, onClaim }: { graph: ChainGraph; preferredProcessId?: string; preferredClaimId?: string; selectedId?: string; relatedClaimsByProcess: Map<string, ChainClaim[]>; onSelect: (node: ChainNode) => void; onClaim: (claimId: string) => void }) {
  const { t } = useTranslation();
  ...
  const flowNodeIds = new Set(flows.flatMap(flow => [flow.process.id, ...flow.inputs.map(node => node.id), ...flow.outputs.map(node => node.id), ...flow.parents.map(node => node.id)]));
  ...
  return <div className={styles.executionFlows}>
    ...
    {flows.map(flow => {
      ...
      const uniqueRelatedClaims = relatedClaims.filter((claim, index, candidates) => candidates.findIndex(candidate => candidate.label === claim.label && candidate.value === claim.value) === index);
      ...
      const focusedClaim = flow.process.id === preferredProcessId ? displayedClaims.find(claim => claim.id === preferredClaimId) : undefined;
      const focusedClaims = focusedClaim ? [focusedClaim] : displayedClaims;
      const otherClaims = focusedClaim ? displayedClaims.filter(claim => claim.id !== focusedClaim.id) : [];
      const focusedOutputs = focusedClaim ? flow.outputs.filter(node => node.label === focusedClaim.label || (node.value !== undefined && focusedClaim.value !== undefined && String(node.value) === String(focusedClaim.value))) : [];
      const primaryLabels = new Set(primaryClaims.map(claim => claim.label));
      const preferredOutputs = focusedOutputs.length ? focusedOutputs : primaryLabels.size ? flow.outputs.filter(node => primaryLabels.has(node.label)) : flow.outputs.slice(0, 3);
      const visibleOutputs = preferredOutputs.length ? preferredOutputs : flow.outputs.slice(0, 3);
      ...
      return <details key={flow.process.id} role="region" aria-label={t("bknTrace.evidenceChain.businessFlowLabel", { title: flow.process.label || accessibleTitle })} className={`${styles.businessFlow} ${selectedId === flow.process.id ? styles.selectedFlow : ""}`} open={expanded.has(flow.process.id)}>
        <summary onClick={event => { event.preventDefault(); onSelect(flow.process); setExpanded(current => { const next = new Set(current); if (next.has(flow.process.id)) next.delete(flow.process.id); else next.add(flow.process.id); return next; }); }}><div><small>{t("bknTrace.evidenceChain.businessTask")}</small><h4>{flow.process.label || accessibleTitle}</h4></div><div className={styles.flowSummary}><strong>{focusedClaim?.label ?? (primaryClaims.length ? primaryClaims.map(claim => claim.label).join("；") : t("bknTrace.evidenceChain.resultCount", { count: flow.outputs.length }))}</strong><span>{t("bknTrace.evidenceChain.flowSummary", { inputs: flow.inputs.length, outputs: flow.outputs.length })}</span></div><em>{t(`bknTrace.evidenceChain.status.${flow.process.status ?? "unknown"}`)}</em></summary>
        {focusedClaim && secondaryOutputs.length > 0 && <p className={styles.focusedFlowNotice}>{t("bknTrace.evidenceChain.focusedExecutionNotice", { count: secondaryOutputs.length })}</p>}
        <div className={styles.flowBody}>
          ...
          <section className={`${styles.flowNode} ${styles.flowOutput}`}><small>{t("bknTrace.evidenceChain.businessResults")}</small><h5>{t("bknTrace.evidenceChain.recordedResults")}</h5>{visibleOutputs.map(node => <BusinessValue key={node.id} node={node} />)}{secondaryOutputs.length > 0 && <details className={styles.secondaryOutputs}><summary>{t("bknTrace.evidenceChain.otherReturnedFields", { count: secondaryOutputs.length })}</summary>{secondaryOutputs.map(node => <BusinessValue key={node.id} node={node} />)}</details>}</section>
        </div>
        {focusedClaims.length > 0 && <nav className={styles.relatedClaims} aria-label={t("bknTrace.evidenceChain.relatedConclusions")}>
          {focusedClaims.map(claim => <button type="button" key={claim.id} onClick={() => onClaim(claim.id)}>{t("bknTrace.evidenceChain.viewRelatedClaim", { label: claim.label })}</button>)}
          {otherClaims.length > 0 && <details className={styles.relatedClaimDetails}><summary>{t("bknTrace.evidenceChain.otherRelatedConclusions", { count: otherClaims.length })}</summary><div>{otherClaims.map(claim => <button type="button" key={claim.id} onClick={() => onClaim(claim.id)}>{t("bknTrace.evidenceChain.viewRelatedClaim", { label: claim.label })}</button>)}</div></details>}
        </nav>}
        {flow.parents.length > 0 && <details className={styles.orchestrators}><summary>{t("bknTrace.evidenceChain.orchestrators", { count: flow.parents.length })}</summary><div>{flow.parents.map(parent => <button key={parent.id} onClick={() => onSelect(parent)}><span>{parent.label}</span><small>{t(`bknTrace.evidenceChain.status.${parent.status ?? "unknown"}`)}</small></button>)}</div></details>}
      </details>;
    })}
...
function RecordedEvidencePath({ claim, graph }: { claim: ChainClaim; graph: ChainGraph }) {
  ...
  const processes = path.nodes.filter(node => node.role === "process" || (!node.role && ["relation", "query", "function", "calculation"].includes(node.kind)));
  if (!processes.length) return null;
  return <div className={styles.derivationPath}>
    <section><small>{t("bknTrace.evidenceChain.objectAndConditions")}</small><h5>{claim.objectLabel ?? t("bknTrace.evidenceChain.recordedObject")}</h5>{inputs.map(node => <BusinessValue key={node.id} node={node} />)}</section>
    <b>→</b><section className={styles.derivationProcess}><small>{t("bknTrace.evidenceChain.businessProcessing")}</small>{processes.map(node => <BusinessValue key={node.id} node={node} />)}</section>
    <b>→</b><section className={styles.derivationResult}><small>{t("bknTrace.evidenceChain.derivation.recordedResult")}</small><h5>{claim.label}</h5><strong>{claim.value}</strong></section>
...
function EvidencePaths({ claims, selectedId, graph, allGraph, metadata, setMetadata, onNode, onEdge }: { claims: ChainClaim[]; selectedId?: string; graph:ChainGraph; allGraph:ChainGraph; metadata:boolean; setMetadata:(value:boolean)=>void; onNode:(node:ChainNode)=>void; onEdge:(edge:ChainEdge)=>void }) {
  const { t } = useTranslation();
  ...
  const recorded = derivation ? null : <RecordedEvidencePath claim={selectedClaim} graph={allGraph} />;
  return <div className={styles.evidencePaths}>
    <p className={styles.executionIntro}>{t("bknTrace.evidenceChain.evidenceIntro")}</p>
    <details role="region" aria-label={t("bknTrace.evidenceChain.claimPath", { label: selectedClaim.label })} className={styles.evidenceClaim} open={Boolean(derivation)}>
        <summary>
          <div><small>{t(selectedClaim.role === "scope" ? "bknTrace.evidenceChain.answerScope" : "bknTrace.evidenceChain.answerConclusion")}</small><h4>{selectedClaim.label}</h4><span>{selectedClaim.objectLabel}</span></div><strong>{selectedClaim.value}</strong><ClaimStates claim={selectedClaim} />
        </summary>
        <div className={styles.derivationBody}>{derivation ? <>
          <div className={styles.derivationPath}>
            <section><small>{t("bknTrace.evidenceChain.objectAndConditions")}</small><h5>{selectedClaim.objectLabel ?? t("bknTrace.evidenceChain.recordedObject")}</h5>{derivation.inputs.map(item => <BusinessValue key={`${item.label}:${item.value}`} node={{id:item.label,label:item.label,value:item.value,kind:"field"}} />)}</section>
            ...
            <b>→</b><section className={styles.derivationResult}><small>{t(`bknTrace.evidenceChain.derivation.${derivation.method === "sum" ? "independentVerification" : derivation.method === "distinct_count" ? "returnedComponents" : "recordedResult"}`)}</small><h5>{derivation.formula || `${selectedClaim.label} = ${selectedClaim.value ?? ""}`}</h5></section>
            <b>→</b><section className={styles.derivationConclusion}><small>{t(selectedClaim.role === "scope" ? "bknTrace.evidenceChain.adoptedScope" : "bknTrace.evidenceChain.adoptedConclusion")}</small><h5>{selectedClaim.label}</h5><strong>{selectedClaim.value}</strong></section>
          </div>
          {derivation.components.length ? <details className={styles.componentDetails}><summary>{t("bknTrace.evidenceChain.viewComponents", { count: derivation.components.length })}</summary><div>{derivation.components.map((item,index) => <div key={`${item.label}:${item.value}:${index}`}><span>{item.label}</span><strong>{item.value}</strong></div>)}</div></details> : null}
        ...
        </> : <>{recorded}<p className={styles.evidenceBoundary}><strong>{t("bknTrace.evidenceChain.evidenceBoundary")}</strong>{selectedClaim.detail ?? t(`bknTrace.evidenceChain.explain.${selectedClaim.status}`)}</p></>}</div>
      </details>
    <section className={styles.focusedEvidence}>
      <header><div><small>{t("bknTrace.evidenceChain.claimEvidence")}</small><h4>{selectedClaim.label}</h4></div></header>
      <div className={styles.toolbar}><span>{t("bknTrace.evidenceChain.interactHint")}</span><label><input type="checkbox" checked={metadata} onChange={event => setMetadata(event.target.checked)} />{t("bknTrace.evidenceChain.metadata")}</label></div>
      <Graph graph={graph} title={t("bknTrace.evidenceChain.evidenceGraph")} mode="evidence" selectedId={undefined} onSelect={onNode} onEdge={onEdge} />
    </section>
    <section className={styles.rawEvidence}>
      <button type="button" className={styles.rawEvidenceToggle} aria-expanded={allFactsOpen} onClick={() => setAllFactsOpen(open => !open)}>{t("bknTrace.evidenceChain.otherEvidence")}</button>
...
function answerSupportStatus(view: EvidenceChainView): "supported" | "partial" | "unsupported" | "contradicted" {
  if (view.requirements?.length) {
    ...
    if (requirementStates.every(state => state === "supported")) return view.evidenceStatus && view.evidenceStatus !== "complete" ? "partial" : "supported";
    if (requirementStates.some(state => state === "supported" || state === "partial")) return "partial";
    return "unsupported";
  }
...
function Panels({ view, initialPanel = "evidence", panel: controlledPanel, onPanelChange }: { view: EvidenceChainView; initialPanel?: "evidence" | "execution"; panel?: "evidence" | "execution"; onPanelChange?: (panel: "evidence" | "execution") => void }) {
  ...
  const recordedClaimGraph = claim ? upstream(view.evidence, claim.nodeIds) : factMode ? { nodes: [], edges: [] } : view.evidence;
  const fullGraph = claim ? {
    nodes: [{ id: `claim:${claim.id}`, label: claim.label, value: claim.value, kind: "conclusion" as const, role: "conclusion" as const, status: claim.supportStatus ?? claim.status, detail: claim.detail }, ...recordedClaimGraph.nodes],
    edges: [...claim.nodeIds.filter(id => recordedClaimGraph.nodes.some(node => node.id === id)).map(id => ({ id: `claim:${claim.id}:${id}`, source: `claim:${claim.id}`, target: id, label: t("bknTrace.evidenceChain.claimBinding"), kind: "binding" as const })), ...recordedClaimGraph.edges],
  } : recordedClaimGraph;
  ...
  const evidenceNode = (id: string) => fullGraph.nodes.find(node => node.id === id) ?? view.evidence.nodes.find(node => node.id === id);
  const preferredClaim = claim ?? view.claims.find(candidate => candidate.role !== "scope") ?? view.claims[0];
  const preferredProcessId = preferredExecutionProcess(view.evidence, view.execution, preferredClaim);
  const relatedClaimsByProcess = new Map<string, ChainClaim[]>();
  for (const candidate of view.claims) {
    const processIds = new Set(upstream(view.evidence, candidate.nodeIds).nodes.map(candidateNode => candidateNode.executionNodeId).filter((id): id is string => Boolean(id)));
    for (const processId of processIds) relatedClaimsByProcess.set(processId, [...(relatedClaimsByProcess.get(processId) ?? []), candidate]);
  }
  ...
  return <ViewportPortal active={fullScreen}><div className={`${styles.root} ${fullScreen ? styles.fullScreen : ""}`} role={fullScreen ? "dialog" : undefined} aria-modal={fullScreen || undefined} aria-label={fullScreen ? fullScreenLabel : undefined}>
    ...
    <section className={styles.summary}>
      ...
      {factMode && <section className={styles.trustWarning} role="status" aria-label={t("bknTrace.evidenceChain.unverifiedAnswerTitle")}>
      ...
      <div className={styles.trustSummary}>
        <div><span>{t("bknTrace.evidenceChain.answerSupport")}</span><strong data-state={supportStatus}>{t(`bknTrace.evidenceChain.answerSupportStatus.${supportStatus}`)}</strong></div>
        <div><span>{t("bknTrace.evidenceChain.evidenceRecord")}</span><strong>{t(`bknTrace.evidenceChain.evidenceRecordStatus.${view.evidenceStatus ?? "unknown"}`)}</strong></div>
        <div><span>{t("bknTrace.evidenceChain.interactionExecution")}</span><strong>{t(`bknTrace.evidenceChain.status.${view.status}`)}</strong></div>
      </div>
      <h4>{t("bknTrace.evidenceChain.answer")}</h4>
      {view.answer?.trim() ? collapseAnswer
        ? <div className={styles.answerPreview}>
          {selectedAnswerExact && <><small>{t("bknTrace.evidenceChain.selectedAnswerAnchor")}</small><button type="button" className={styles.anchor} aria-pressed="true" onClick={() => claimId && selectClaim(claimId)}>{selectedAnswerExact}</button></>}
          <button type="button" className={styles.answerToggle} aria-expanded="false" onClick={() => setAnswerExpanded(true)}>{t("bknTrace.evidenceChain.viewOriginalAnswer")}</button>
        </div>
        : <><Answer text={view.answer} claims={view.claims} select={selectClaim} selectedId={claimId} />{answerIsLong && <button type="button" className={styles.answerToggle} aria-expanded="true" onClick={() => setAnswerExpanded(false)}>{t("bknTrace.evidenceChain.collapseOriginalAnswer")}</button>}</>
    ...
    {panel === "evidence" && !factMode && <aside className={styles.catalog}><h3>{t(propositionMode ? "bknTrace.evidenceChain.questionRequirements" : "bknTrace.evidenceChain.conclusions")}</h3>{propositionMode ? <>
      {view.requirements?.map(requirement => {
        const matchingClaims = primaryClaims.filter(claim => requirement.claimIds.includes(claim.id));
        const displayedClaims = matchingClaims.filter(claim => primaryRequirementOwner.get(claim.id) === requirement.id);
        return <section className={styles.objectGroup} key={requirement.id}><h4>{requirement.label}</h4><small>{t(`bknTrace.evidenceChain.requirementStatus.${requirement.status}`)}</small><div className={styles.claims}>{displayedClaims.map(claim => <button key={claim.id} aria-pressed={claimId === claim.id} onClick={() => selectClaim(claim.id)}><span>{claim.label}</span><ClaimStates claim={claim} /></button>)}</div>{matchingClaims.length > displayedClaims.length && <p className={styles.sharedCoverage}>{t("bknTrace.evidenceChain.sharedRequirementCoverage")}</p>}</section>;
      })}
...
export function EvidenceChainPanels({ view, initialPanel, panel, onPanelChange }: { view: EvidenceChainView; initialPanel?: "evidence" | "execution"; panel?: "evidence" | "execution"; onPanelChange?: (panel: "evidence" | "execution") => void }) {
  return <Panels key={`${view.interactionId}:${view.revisionLabel ?? ""}`} view={view} initialPanel={initialPanel} panel={panel} onPanelChange={onPanelChange} />;
}
