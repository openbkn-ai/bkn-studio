...
function ExecutionFlows({ graph, preferredProcessId, selectedId, onSelect }: { graph: ChainGraph; preferredProcessId?: string; selectedId?: string; onSelect: (node: ChainNode) => void }) {
  const { t } = useTranslation();
    ...
    {flows.map(flow => {
      ...
      return <details key={flow.process.id} role="region" aria-label={t("bknTrace.evidenceChain.businessFlowLabel", { title })} className={`${styles.businessFlow} ${selectedId === flow.process.id ? styles.selectedFlow : ""}`} open={expanded.has(flow.process.id)}>
        ...
        <div className={styles.flowBody}>
          ...
          <section className={`${styles.flowNode} ${styles.flowOutput}`}><small>{t("bknTrace.evidenceChain.businessResults")}</small><h5>{t("bknTrace.evidenceChain.recordedResults")}</h5>{flow.outputs.map(node => <BusinessValue key={node.id} node={node} />)}</section>
        </div>
      </details>;
    })}
...
function EvidencePaths({ claims, selectedId, select, graph, allGraph, metadata, setMetadata, onNode, onEdge }: { claims: ChainClaim[]; selectedId?: string; select:(id:string)=>void; graph:ChainGraph; allGraph:ChainGraph; metadata:boolean; setMetadata:(value:boolean)=>void; onNode:(node:ChainNode)=>void; onEdge:(edge:ChainEdge)=>void }) {
  ...
  return <div className={styles.evidencePaths}>
    ...
    <section className={styles.rawEvidence}>
      ...
      {allFactsOpen && <><p className={styles.rawEvidenceScope}>{t("bknTrace.evidenceChain.rawEvidenceAllScope")}</p><Graph graph={allGraph} title={t("bknTrace.evidenceChain.allFactsGraph")} mode="evidence" selectedId={undefined} onSelect={onNode} onEdge={onEdge} /></>}</section>
  </div>;
}
