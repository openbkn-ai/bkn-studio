...
describe("semantic graph layout", () => {
  ...
  it("has no overlapping cards, including multiple objects and calculations", () => {
    const expanded = {
      ...graph,
      nodes: Array.from({ length: 30 }, (_, i) => ({ ...graph.nodes[i % 5], id: `n${i}` })),
    };
  ...
  it("places explicit execution inputs, process and outputs in different columns", () => {
    const result = layoutChain(
      {
        nodes: [
          { id: "out", label: "Result", kind: "field", role: "output" },
          { id: "in", label: "Demand", kind: "field", role: "input" },
          { id: "fn", label: "Evaluation", kind: "function" },
        ],
        edges: [],
      },
      "execution",
