...
export function scopeObjectFacts(
  graph: ChainGraph,
  objectId: string,
  fieldId?: string,
): ChainGraph {
    ...
    graph.edges
      .filter(
        (e) =>
          e.kind === "object" &&
          e.source === objectId &&
          graph.nodes.some((n) => n.id === e.target && n.kind === "field"),
      )
      .map((e) => e.target),
  );
  ...
  for (const edge of graph.edges)
    if (
      edge.kind === "value" &&
      chosen.has(edge.target) &&
      graph.nodes.some((n) => n.id === edge.source && n.kind === "source")
    )
      ids.add(edge.source);
  if (!fieldId) {
    const relationships = new Set(
      graph.edges
        .filter((e) => e.kind === "relation" && (e.source === objectId || e.target === objectId))
        .flatMap((e) => [e.source, e.target])
        .filter((id) => graph.nodes.some((n) => n.id === id && n.kind === "relation")),
    );
    for (const id of relationships) ids.add(id);
    for (const e of graph.edges)
      if (e.kind === "relation" && (relationships.has(e.source) || relationships.has(e.target))) {
        for (const id of [e.source, e.target])
          if (
            graph.nodes.some((n) => n.id === id && (n.kind === "object" || n.kind === "relation"))
          )
            ids.add(id);
      }
  }
...
export function scopeSourceFacts(
  graph: ChainGraph,
  sourceId: string,
  fieldId?: string,
): ChainGraph {
  ...
  return {
    ...
    edges: graph.edges.filter(
      (e) => e.source === sourceId && e.kind === "value" && ids.has(e.target),
    ),
  };
}
