...
function pairGraph(
  ...
  pair?: QuestionPairView,
): PairProvenanceGraph | undefined {
...
function QuestionPairCards({
  pairs,
  selectedId,
  onSelect,
}: {
  pairs: QuestionPairView[];
  selectedId?: string;
  onSelect: (id: string) => void;
}) {
  return (
    <aside className={styles.pairPanel} aria-label={p16Text("pairs.title")}>
      <header>
        <div>
          <strong>{p16Text("pairs.title")}</strong>
          <Tag>{pairs.length}</Tag>
        </div>
        <p>{p16Text("pairs.description")}</p>
      </header>
        ...
        {pairs.map((pair, index) => (
          ...
          >
            ...
            <span>
              <b>{pair.summary}</b>
              <small>
                {pair.answer ? answerSummary(pair.answer) : p16Text("pairs.answerMissing")}
              </small>
            </span>
...
function payloadContent(value?: TimeRailPayload) {
  ...
  if (value.mode === "inline" && value.inline !== undefined)
    return typeof value.inline === "string" ? value.inline : JSON.stringify(value.inline, null, 2);
...
function BusinessFunctionCard({
  ...
  value: AttributedBusinessFunctionView;
  ...
  return (
    <article className={styles.functionCard}>
      <header>
        <div>
          <span className={styles.kicker}>{p16Text("function.title")}</span>
      ...
      <div className={styles.functionFlow}>
        <section>
          <span>{p16Text("function.input")}</span>
          {value.businessInputs.length ? (
            ...
            <p>{p16Text("function.inputMissing")}</p>
          )}
        </section>
        <i aria-hidden="true">→</i>
        <section>
          <span>{p16Text("function.process")}</span>
          <b>{value.displayName}</b>
          <p>{value.logicSummary}</p>
        </section>
        <i aria-hidden="true">→</i>
        <section>
      ...
      >
        {technical
          ? p16Text("function.collapseTechnical")
          : p16Text("function.expandTechnical", {
              count: value.operationIds.length || value.technicalExecution.interfaceNames.length,
            })}
      </button>
      {technical && (
        <section
          className={styles.technicalDetails}
          aria-label={p16Text("function.technicalLabel", { name: value.displayName })}
        >
          <dl>
            <div>
              <dt>{p16Text("function.realInterface")}</dt>
              <dd>
                {value.technicalExecution.interfaceNames.length
                  ? value.technicalExecution.interfaceNames.map((name) => (
                      <code key={name}>{name}</code>
                    ))
                  : p16Text("function.notRecorded")}
              </dd>
            </div>
            <div>
              <dt>{p16Text("function.operationId")}</dt>
              <dd>
                {value.operationIds.length
                  ? value.operationIds.map((id) => <code key={id}>{id}</code>)
                  : p16Text("function.notRecorded")}
              </dd>
            </div>
            <div>
              <dt>{p16Text("function.inputPayload")}</dt>
              <dd>
                {value.technicalExecution.inputPayloadRef || p16Text("function.inlineOrMissing")}
              </dd>
            </div>
            <div>
              <dt>{p16Text("function.outputPayload")}</dt>
              <dd>
                {value.technicalExecution.outputPayloadRef || p16Text("function.inlineOrMissing")}
              </dd>
            </div>
            <div>
              <dt>{p16Text("function.completeness")}</dt>
              <dd>{value.technicalExecution.completeness || p16Text("function.notRecorded")}</dd>
            </div>
          </dl>
          {attempts.map((item) => (
...
function ReadingView({
  ...
  const visibleEvidence = selectedClaim
    ? graph.evidenceNodes.filter((node) => evidenceIds.has(node.id))
    : graph.evidenceNodes;
  return (
    <div className={styles.reading}>
      <section className={styles.answerCard}>
        <div>
          <span className={styles.kicker}>{p16Text("reading.currentQuestion")}</span>
          <h2>{pair.summary}</h2>
          <p>{pair.question}</p>
        </div>
        <div className={styles.answer}>
          <span>{p16Text("reading.answer")}</span>
          <div className={styles.answerSummary}>
            <MarkdownText text={summary} variant="document" />
          </div>
          <div className={styles.answerActions}>
            <em className={styles[pair.status] || styles.neutral}>{statusLabel(pair.status)}</em>
            {pair.answer && pair.answer !== summary ? (
              <button type="button" onClick={() => setAnswerOpen((current) => !current)}>
                {answerOpen ? p16Text("reading.collapseAnswer") : p16Text("reading.expandAnswer")}
              </button>
            ) : null}
          </div>
        </div>
      ...
      {answerOpen ? (
        <section className={styles.answerDocument} aria-label={p16Text("reading.fullAnswer")}>
          <MarkdownText text={pair.answer} variant="document" />
        </section>
      ) : null}
      {graph.claims.length > 0 && (
        <section className={styles.claimTableWrap}>
          <header>
            <div>
              <span className={styles.kicker}>{p16Text("reading.conclusion")}</span>
              <h3>{p16Text("reading.memberConclusions", { count: graph.claims.length })}</h3>
            </div>
            <Tag color="success">
              {p16Text("reading.supportedCount", {
                count: graph.claims.filter((claim) => claim.supportStatus === "supported").length,
              })}
            </Tag>
          </header>
          <div className={styles.claimTableScroller}>
            <table className={styles.claimTable} aria-label={p16Text("reading.conclusion")}>
              <thead>
                <tr>
                  <th>#</th>
                  <th>{p16Text("reading.conclusion")}</th>
                  <th>{p16Text("reading.value")}</th>
                  <th>{p16Text("reading.state")}</th>
                </tr>
              </thead>
              <tbody>
                {graph.claims.map((claim, index) => (
                  <tr
                    key={claim.id}
                    className={claim.id === selectedClaim?.id ? styles.claimSelected : undefined}
                  >
                    <td>{index + 1}</td>
                    <td>
                      <button
                        ...
                        aria-label={p16Text("reading.selectConclusion", { label: claim.label })}
                    ...
                    <td>
                      <span
                        className={`${styles.claimState} ${styles[claim.supportStatus || "neutral"]}`}
                      >
                        {statusLabel(claim.supportStatus)}
                      </span>
                    </td>
                  </tr>
                ))}
      ...
      {visibleFunctions.length ? (
        visibleFunctions.map((item) => (
          <BusinessFunctionCard key={item.id} value={item} timeRail={timeRail} />
        ))
      ) : (
...
function graphNodes(
  ...
  graph.businessFunctions.forEach((value, index) =>
    nodes.push({
      id: value.id,
      type: "function",
      title: value.displayName,
      copy: value.logicSummary,
      meta: capabilityLabel(value.capabilityKind),
      x: 28,
      y: 52 + index * 150,
    }),
  );
  if (layers.has("technical"))
    graph.executionSteps.forEach((value, index) =>
      nodes.push({
        id: value.id,
        type: "step",
        title: value.businessRole || value.interfaceName,
  ...
  const selectedClaim =
    graph.claims.find((value) => value.id === selectedClaimId) || graph.claims[0];
  if (selectedClaim)
    nodes.push({
      id: selectedClaim.id,
      type: "claim",
...
function GraphView({
  ...
  const nodes = useMemo(
    () => graphNodes(graph, layers, selectedClaimId),
    [graph, layers, selectedClaimId],
  );
  const [selectedId, setSelectedId] = useState<string>(
    selectedClaimId || graph.businessFunctions[0]?.id,
  );
  useEffect(
    () => setSelectedId(selectedClaimId || graph.businessFunctions[0]?.id),
  ...
  const connected = useMemo(() => {
    ...
    while (changed) {
      ...
      edges.forEach((edge) => {
        ...
        if (ids.has(edge.toId) && !ids.has(edge.fromId)) {
          ids.add(edge.fromId);
          changed = true;
        }
      });
  ...
  return (
    <section className={styles.graphPanel}>
      <header className={styles.graphHeader}>
        <div>
          <span className={styles.kicker}>{p16Text("graph.kicker")}</span>
          <h2>{p16Text("graph.title")}</h2>
          <p>{p16Text("graph.description")}</p>
        </div>
      </header>
      <div className={styles.graphToolbar}>
        <label className={styles.graphClaimPicker}>
          <span>{p16Text("graph.selectClaim")}</span>
          <select
            aria-label={p16Text("graph.selectClaim")}
        ...
        <button type="button" className={styles.layerActive}>
          {p16Text("graph.businessLayer")}
        </button>
        <button
          type="button"
          className={layers.has("ontology") ? styles.layerActive : undefined}
          onClick={() => toggleLayer("ontology")}
          aria-pressed={layers.has("ontology")}
        >
          {p16Text("graph.ontologyLayer")}
        </button>
        <button
          type="button"
          className={layers.has("technical") ? styles.layerActive : undefined}
          onClick={() => toggleLayer("technical")}
          aria-pressed={layers.has("technical")}
        >
          {p16Text("graph.technicalLayer")}
        </button>
      </div>
      <div className={styles.graphLayout}>
            ...
            {edges.map((edge: ProvenanceEdgeView) => {
              ...
              return (
                <path
                  key={edge.edgeId}
                  className={
                    selected && connected.has(from.id) && connected.has(to.id)
                      ? styles.edgeActive
                      : undefined
                  }
                  d={`M ${x1} ${y1} C ${mid} ${y1}, ${mid} ${y2}, ${x2} ${y2}`}
                  markerEnd="url(#bp016-arrow)"
                />
              );
            })}
          </svg>
        ...
        <aside className={styles.inspector} aria-label={p16Text("graph.inspector")}>
          {selected ? (
            <>
              <span className={styles.kicker}>{nodeTypeLabel(selected.type)}</span>
              <h3>{selected.title}</h3>
              <p>{selected.copy || p16Text("graph.noDetail")}</p>
              <dl>
                <div>
                  <dt>{p16Text("graph.projectedNode")}</dt>
                  <dd>
                    <code>{selected.id}</code>
                  </dd>
                </div>
                <div>
                  <dt>{p16Text("graph.source")}</dt>
                  <dd>{selected.meta || nodeTypeLabel(selected.type)}</dd>
                </div>
                {connected.size > 1 && (
                  <div>
                    <dt>{p16Text("graph.proofPath")}</dt>
                    <dd>
                      {nodes
                        .filter((node) => node.id !== selected.id && connected.has(node.id))
                        .map((node) => (
                          <span className={styles.pathNode} key={node.id}>
                            {nodeTypeLabel(node.type)} · {node.title}
                          </span>
                        ))}
                    </dd>
                  </div>
              ...
              {selected.type === "function" && (
                <button type="button" onClick={() => toggleLayer("technical")}>
                  {layers.has("technical")
                    ? p16Text("graph.collapseExecution")
                    : p16Text("graph.expandExecution")}
                </button>
              )}
            </>
          ) : (
            <p>{p16Text("graph.selectNode")}</p>
          )}
        </aside>
      </div>
    </section>
...
function payloadSummary(value?: TimeRailPayload) {
  if (!value) return p16Text("function.notRecorded");
  if (value.mode === "referenced")
    return p16Text("timeline.referenced", { bytes: value.byte_length });
  ...
  if (
    typeof value.inline === "number" ||
    typeof value.inline === "boolean" ||
    typeof value.inline === "bigint"
  )
    return String(value.inline);
...
function TimeRailView({ items }: { items: TimeRailItem[] }) {
  ...
  return (
    <section className={styles.timeWorkspace}>
      <header className={styles.timeIntro}>
        <div>
          <span className={styles.kicker}>{p16Text("timeline.kicker")}</span>
          <h2>{p16Text("timeline.title")}</h2>
          <p>{p16Text("timeline.description")}</p>
        </div>
        <div
          className={styles.timeFilters}
          role="group"
          aria-label={p16Text("timeline.filterLabel")}
        >
          <button
            type="button"
          ...
          <button
            type="button"
            className={filter === "completed" ? styles.filterActive : undefined}
          ...
          <button
            type="button"
            className={filter === "failed" ? styles.filterActive : undefined}
...
export function BusinessProvenance016({
  view,
  panel = "evidence",
  onPanelChange,
  evidenceOnly = false,
  ...
  useEffect(
    () => setSelectedId(firstPairId),
    [view.interactionId, view.revisionLabel, firstPairId],
  );
  ...
  const content = (
    <div className={`${styles.shell} ${fullScreen ? styles.fullScreen : ""}`}>
      <header className={styles.workspaceToolbar}>
        {!evidenceOnly ? (
          <div className={styles.primaryTabs} role="tablist" aria-label={p16Text("workspace.tabs")}>
            <button
              type="button"
              role="tab"
              aria-selected={activePanel === "timeline"}
              className={activePanel === "timeline" ? styles.primaryActive : undefined}
              onClick={() => selectPanel("timeline")}
            >
              {p16Text("workspace.timeline")}
            </button>
            <button
              type="button"
              role="tab"
              aria-selected={activePanel === "evidence"}
              className={activePanel === "evidence" ? styles.primaryActive : undefined}
              onClick={() => selectPanel("evidence")}
            >
              {p16Text("workspace.evidence")}
            </button>
          </div>
        ) : (
          <div>
        ...
        <button
          ...
          aria-label={
            fullScreen ? p16Text("workspace.exitFullScreen") : p16Text("workspace.fullScreen")
          }
      ...
      {!evidenceOnly ? (
        <>
          <section className={styles.questionBanner}>
            <span>Q</span>
            <div>
              <small>{p16Text("workspace.userQuestion")}</small>
              <strong>
                {view.question || selectedPair?.question || p16Text("pairs.answerMissing")}
              </strong>
            </div>
          </section>
          <section className={styles.summaryGrid} aria-label={p16Text("workspace.summary")}>
            <article>
              <span>{p16Text("workspace.pairs")}</span>
              <b>{pairs.length}</b>
              <small>{p16Text("workspace.byQuestion")}</small>
            </article>
            <article>
              <span>{p16Text("workspace.supported")}</span>
              <b>{supported}</b>
              <small>{p16Text("workspace.ofClaims", { count: view.claims.length })}</small>
            </article>
            <article>
              <span>{p16Text("workspace.businessCalls")}</span>
              <b>
                {
                  new Set(
                    Object.values(view.selectedPairGraphs || {}).flatMap((value) =>
                      value.executionSteps.map((step) => step.operationId),
                    ),
                  ).size
                }
              </b>
              <small>{p16Text("workspace.usedByAnswers")}</small>
            </article>
            <article>
              <span>{p16Text("workspace.recordedCalls")}</span>
              <b>{(view.timeRail || []).length}</b>
              <small>{p16Text("workspace.failedCalls", { count: failed })}</small>
            </article>
          </section>
        </>
      ) : null}
      {!evidenceOnly && activePanel === "timeline" ? (
        <TimeRailView items={view.timeRail || []} />
      ) : (
        <>
          {view.generationStatus === "analysis_pending" && (
            <Alert
              type="info"
              showIcon
              message={p16Text("pending.title")}
              description={p16Text("pending.description")}
            />
          )}
          {!selectedPair ? (
            <Alert
              type="warning"
              showIcon
              message={p16Text("pending.missingGraph")}
              description={p16Text("pending.missingGraphDescription")}
            />
          ) : (
                ...
                {!graph ? (
                  ...
                  <>
                    <div className={styles.modebar}>
                      <div role="group" aria-label={p16Text("modes.label")}>
                        <button
                          type="button"
                          className={mode === "reading" ? styles.modeActive : undefined}
                          onClick={() => setMode("reading")}
                        >
                          {p16Text("modes.reading")}
                        </button>
                        <button
                          type="button"
                          className={mode === "graph" ? styles.modeActive : undefined}
                          onClick={() => setMode("graph")}
                        >
                          {p16Text("modes.graph")}
                        </button>
                      </div>
                      <span>{p16Text("modes.description")}</span>
                      <em>{p16Text("modes.persistedOnly")}</em>
                    </div>
                    {mode === "reading" ? (
                      <ReadingView
                        pair={selectedPair}
                        graph={graph}
                        timeRail={view.timeRail || []}
