...
function BusinessFunctionCard({ value, timeRail }: { value: AttributedBusinessFunctionView; timeRail: TimeRailItem[] }) {
  ...
  return <article className={styles.functionCard}>
    <header>
      ...
      <Tag color={value.validationStatus === "invalid" ? "error" : "blue"}>{capabilityLabel(value.capabilityKind)}</Tag>
    </header>
    <p className={styles.purpose}>{value.businessPurpose}</p>
    <div className={styles.functionFlow}>
      <section><span>{p16Text("function.input")}</span>{value.businessInputs.length ? value.businessInputs.map((input, index) => <div key={`${input.name}:${index}`}><b>{input.name}</b><p>{input.value || input.sourceRef || p16Text("function.valueMissing")}</p><small>{input.sourceKind ? p16Text("function.source", { source: input.sourceKind }) : ""}</small></div>) : <p>{p16Text("function.inputMissing")}</p>}</section>
      <i aria-hidden="true">→</i>
      <section><span>{p16Text("function.process")}</span><b>{value.displayName}</b><p>{value.logicSummary}</p></section>
      <i aria-hidden="true">→</i>
      <section><span>{p16Text("function.output")}</span>{value.businessOutputs.length ? value.businessOutputs.map((output, index) => <p key={`${output.summary}:${index}`}>{output.summary}</p>) : <p>{p16Text("function.outputMissing")}</p>}</section>
    </div>
    ...
    {technical && <section className={styles.technicalDetails} aria-label={p16Text("function.technicalLabel", { name: value.displayName })}>
      ...
      {attempts.map(item => <article className={styles.attempt} key={item.id}>
        <header><code>{item.interface_name}</code><span>{item.operation_id} · #{item.attempt}</span><Tag color={item.status === "completed" ? "success" : item.status === "failed" ? "error" : "default"}>{item.status}</Tag></header>
...
function ReadingView({ pair, graph, timeRail, selectedClaimId, onSelectClaim }: { pair: QuestionPairView; graph: PairProvenanceGraph; timeRail: TimeRailItem[]; selectedClaimId?: string; onSelectClaim: (id: string) => void }) {
  ...
  const visibleFunctions = selectedClaim ? graph.businessFunctions.filter(item => item.supportsClaimIds.includes(selectedClaim.id)) : graph.businessFunctions;
  ...
  return <div className={styles.reading}>
    ...
    {visibleEvidence.length > 0 && <section className={styles.evidenceList}>
      <header><h3>{p16Text("reading.adoptedEvidence")}</h3><span>{p16Text("reading.itemCount", { count: visibleEvidence.length })}</span></header>
...
function graphNodes(graph: PairProvenanceGraph, layers: Set<GraphLayer>, selectedClaimId?: string): VisualNode[] {
  ...
  if (layers.has("technical")) graph.executionSteps.forEach((value, index) => nodes.push({ id: value.id, type: "step", title: value.businessRole || value.interfaceName, copy: value.interfaceName, meta: value.operationId, x: 28, y: 260 + index * 135 }));
  ...
  if (layers.has("ontology")) graph.schemaNodes.forEach((value, index) => nodes.push({ id: value.id, type: "schema", title: value.label, copy: value.detail, meta: value.kind, x: 300, y: 330 + index * 120 }));
  const selectedClaim = graph.claims.find(value => value.id === selectedClaimId) || graph.claims[0];
  if (selectedClaim) nodes.push({ id: selectedClaim.id, type: "claim", title: selectedClaim.label, copy: selectedClaim.value, meta: statusLabel(selectedClaim.supportStatus), x: 300, y: Math.max(190, 62 + graph.evidenceNodes.length * 125) });
  return nodes;
}
...
function TimeRailView({ items }: { items: TimeRailItem[] }) {
  ...
  return <section className={styles.timeWorkspace}>
    <header className={styles.timeIntro}><div><span className={styles.kicker}>{p16Text("timeline.kicker")}</span><h2>{p16Text("timeline.title")}</h2><p>{p16Text("timeline.description")}</p></div><div className={styles.timeFilters} role="group" aria-label={p16Text("timeline.filterLabel")}><button type="button" className={filter === "all" ? styles.filterActive : undefined} onClick={() => setFilter("all")}>{p16Text("timeline.all", { count: ordered.length })}</button><button type="button" className={filter === "completed" ? styles.filterActive : undefined} onClick={() => setFilter("completed")}>{p16Text("timeline.completed", { count: ordered.filter(item => item.status === "completed").length })}</button><button type="button" className={filter === "failed" ? styles.filterActive : undefined} onClick={() => setFilter("failed")}>{p16Text("timeline.failed", { count: ordered.filter(item => item.status === "failed").length })}</button></div></header>
    <div className={styles.timeLayout}>
      <div className={styles.timeList}>{filtered.map((item, index) => <button type="button" key={item.id} className={`${styles.timeItem} ${item.id === selected?.id ? styles.timeSelected : ""}`} onClick={() => setSelectedId(item.id)}><time>{timeLabel(item.started_at)}</time><span className={`${styles.timeDot} ${styles[item.status] || ""}`}>{item.order}</span><span className={styles.timeCopy}><b>{item.interface_name}</b><small>{item.source_module || item.protocol} · {durationLabel(item.duration_ms)}</small><em>{item.status === "failed" ? payloadSummary(item.error) : payloadSummary(item.output)}</em></span>{index < filtered.length - 1 ? <i aria-hidden="true" /> : null}</button>)}</div>
      <aside className={styles.timeInspector} aria-label={p16Text("timeline.inspector")}><span className={styles.kicker}>{p16Text("timeline.selectedCall")}</span><h3>{selected?.interface_name}</h3><p>{selected?.status === "failed" ? p16Text("timeline.failedCall") : payloadSummary(selected?.output)}</p><dl><div><dt>{p16Text("function.operationId")}</dt><dd><code>{selected?.operation_id}</code></dd></div><div><dt>{p16Text("timeline.status")}</dt><dd>{selected?.status}</dd></div><div><dt>{p16Text("timeline.started")}</dt><dd>{timeLabel(selected?.started_at)}</dd></div><div><dt>{p16Text("timeline.duration")}</dt><dd>{durationLabel(selected?.duration_ms)}</dd></div><div><dt>{p16Text("function.realInterface")}</dt><dd><code>{selected?.interface_name}</code></dd></div></dl><details><summary>{p16Text("function.inputPayload")}</summary><pre>{payloadContent(selected?.input)}</pre></details>{selected?.output ? <details><summary>{p16Text("function.outputPayload")}</summary><pre>{payloadContent(selected.output)}</pre></details> : null}{selected?.error ? <details><summary>{p16Text("function.errorPayload")}</summary><pre>{payloadContent(selected.error)}</pre></details> : null}</aside>
    </div>
  </section>;
}
