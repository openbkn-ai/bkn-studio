...
const view: EvidenceChainView = {
  interactionId: "test-only",
  question: "When?",
  answer: "Late 24 days; stock 0.",
  status: "interrupted",
  claims: [
    {
      id: "late",
      ...
      value: "24 days",
      status: "checked",
      answerRange: { start: 5, end: 12, exact: "24 days" },
      nodeIds: ["delay"],
    },
    ...
    nodes: [
      {
        ...
        kind: "query",
        status: "failed",
        technical: "private-op-id",
      },
      { id: "other", label: "Other call", kind: "api" },
    ],
    edges: [],
  },
  evidence: {
    nodes: [
      { id: "date", label: "Available date", kind: "field" },
      { id: "delay", label: "Customer delay", kind: "result" },
      { id: "unrelated", label: "Unrelated material", kind: "object" },
    ],
    edges: [{ id: "d", source: "date", target: "delay", label: "Date difference" }],
  },
};
describe("EvidenceChainPanels", () => {
          ...
          claims: [
            {
              ...
              value: "2 个",
              role: "primary",
              requirementIds: ["inventory"],
              status: "located",
              supportStatus: "supported",
              attributionStatus: "reconstructed",
              answerRange: { start: 0, end: 13, exact: "当前全仓可用库存为 2 个" },
              nodeIds: ["date"],
            },
          ],
  ...
  it("shows a shared conclusion once when it answers multiple requirements", () => {
    const sharedClaim = {
      id: "player",
      label: "1978 年，Mario Kempes 代表阿根廷获得金球奖，并在同届世界杯打进 5 球。",
      value: "1978、阿根廷、5 球",
      role: "primary" as const,
      requirementIds: ["members", "details"],
    ...
    const catalog = screen.getByText("问题要求与关键结论").closest("aside")!;
    expect(within(catalog).getAllByRole("button", { name: /1978 年，Mario Kempes/ })).toHaveLength(
      1,
    );
  ...
  it("shows the selected conclusion evidence graph as the primary workspace", () => {
    ...
    expect(screen.getByRole("button", { name: "查看本轮其他记录" })).toHaveAttribute(
      "aria-expanded",
      "false",
    );
  });
  it("puts the trust decision before a long unverified answer and keeps the answer collapsed", () => {
    ...
    expect(
      warning.compareDocumentPosition(answerHeading) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
          ...
          claims: [
            {
              id: "total",
              label: "可用库存合计",
              value: "100,508,026.49",
              role: "result",
              status: "located",
              nodeIds: ["date"],
              derivation: {
                method: "direct_return",
                processName: "全网可用库存指标",
                boundary: "指标返回总量。",
                inputs: [],
                components: [],
              },
    ...
    render(
      <EvidenceChainPanels
        view={{
          ...
          evidenceStatus: "partial",
          claims: [
            { ...view.claims[0], supportStatus: "supported", attributionStatus: "reconstructed" },
          ],
        }}
      />,
    );
  ...
  it("resolves an all-interaction diagnostic edge against the all-facts graph", () => {
    ...
    expect(screen.getByRole("dialog", { name: "关系详情" })).toHaveTextContent(
      "Stock source → Stock fact",
    );
  });
  ...
  it("switches the focused business explanation while keeping component details optional", () => {
    const explained: EvidenceChainView = {
      ...
      claims: [
        {
          ...
          value: "906",
          status: "located",
          answerRange: { start: 0, end: 8, exact: "可用库存 906" },
          nodeIds: ["inventory-fact"],
          objectLabel: "物料 165-001335",
          derivation: {
            method: "sum",
            processName: "物料可用库存指标",
            formula: "20 + 1 + 77 + 121 + 9 + 1 + 677 = 906",
            verification: "independent_cross_check",
            boundary: "指标内部计算明细未记录；分仓数据来自另一次同口径查询。",
            inputs: [{ label: "数据时点", value: "当前" }],
            components: [
              { label: "乌鲁木齐备件仓 · 可用", value: "20" },
              { label: "苏州无人机原料仓 · 可用", value: "677" },
            ],
          },
        },
        {
          ...
          value: "3",
          status: "located",
          answerRange: { start: 9, end: 18, exact: "反查被 3 个产品" },
          nodeIds: ["products-fact"],
          objectLabel: "物料 165-001335",
          derivation: {
            method: "distinct_count",
            processName: "物料反查产品",
            formula: "去重后的产品编码数 = 3",
            verification: "same_return",
            boundary: "未记录产品级 BOM 回溯明细。",
            inputs: [{ label: "统计口径", value: "BOM 根产品（含替代料）" }],
            components: [
              { label: "产品", value: "367-000061" },
              { label: "产品", value: "367-000130" },
              { label: "产品", value: "367-000131" },
            ],
          },
        },
      ],
      evidence: {
        nodes: [
          { id: "inventory-fact", label: "可用库存", kind: "field", value: "906" },
          { id: "products-fact", label: "使用产品数", kind: "field", value: "3" },
        ],
        edges: [],
      },
    };
    ...
    fireEvent.click(screen.getByRole("button", { name: "查看本轮其他记录" }));
    expect(
      within(screen.getByRole("region", { name: "本轮完整记录事实" })).getByText("使用产品数"),
    ).toBeVisible();
  });
        ...
        nodes: [
          {
            ...
            label: "库存查询",
            kind: "source",
            executionNodeId: "inventory-query",
          },
          {
            id: "product-source",
            label: "产品BOM查询",
            kind: "source",
            executionNodeId: "product-query",
          },
  ...
  it("keeps the recorded function output when an answer claim has a different value", () => {
    const recorded: EvidenceChainView = {
      ...view,
      claims: [
        { id: "claim", label: "仓库数量", value: "29", status: "located", nodeIds: ["fact"] },
      ],
      execution: {
        nodes: [
        ...
        edges: [
          { id: "in", source: "scope", target: "metric", label: "作为输入", kind: "execution" },
          { id: "out", source: "metric", target: "output", label: "返回", kind: "execution" },
        ],
      },
      evidence: {
        nodes: [
          { id: "source", label: "指标返回", kind: "source", executionNodeId: "metric" },
          { id: "fact", label: "仓库数量", value: "29", kind: "field", role: "fact" },
        ],
        edges: [],
      },
    };
  ...
  it("leads an execution step with its process and key conclusions", () => {
    ...
    expect(
      within(flow).queryByRole("button", { name: "查看结论：库存记录位于生产维修仓 →" }),
  ...
  it("focuses a shared business process on the conclusion selected by the user", () => {
    ...
    const results = within(flow)
      .getByRole("heading", { name: "本次返回的结果" })
      .closest("section")!;
    ...
    expect(
      within(flow).getByText(
        "该业务处理同时返回 1 项其他结果。下面优先显示当前结论；左侧条件保持本次调用的真实记录。",
      ),
    ).toBeVisible();
    ...
    render(
      <EvidenceChainPanels
        view={{
          ...view,
          claims: [
            { id: "stock", label: "库存", value: "906", status: "located", nodeIds: ["date"] },
          ],
        }}
      />,
    );
  ...
  it("opens the answer conclusion flow before an earlier scope-only flow", () => {
        ...
        nodes: [
          {
            ...
            label: "全网库存返回",
            kind: "source",
            executionNodeId: "total-inventory",
          },
          {
            id: "scope-source",
            label: "仓库数量返回",
            kind: "source",
            executionNodeId: "warehouse-count",
          },
        ],
        edges: [],
      },
    ...
    expect(screen.getByRole("region", { name: "全网可用库存指标业务处理" })).not.toHaveAttribute(
      "open",
    );
  });
  it("keeps incomplete recorded links out of the business flow and available for troubleshooting", () => {
    const execution: EvidenceChainView["execution"] = {
      nodes: [
        { id: "a", label: "Query A", kind: "query" },
        { id: "x", label: "Result X", kind: "result" },
        { id: "y", label: "Result Y", kind: "result" },
        { id: "b", label: "Query B", kind: "query" },
      ],
      edges: [
  ...
  it("keeps explicitly returned relationship paths in object scope with localized labels", () => {
    const relationView: EvidenceChainView = {
      ...view,
      claims: [
        {
          id: "material",
      ...
      evidence: {
        nodes: [
          { id: "a", label: "Product A", kind: "object" },
          { id: "b", label: "Material B", kind: "object" },
          { id: "r", label: "Recorded BOM relation", kind: "relation", role: "process" },
        ],
        edges: [
          { id: "from", source: "a", target: "r", label: "Uses", kind: "relation" },
    ...
    expect(screen.getByRole("dialog", { name: "关系详情" })).toHaveTextContent(
      "Recorded BOM relation",
    );
  });
  it("does not present unbound raw fields as answer conclusions", () => {
    const facts: EvidenceChainView = {
      ...view,
      claims: [],
      answer: "产品 P61-000351 的成品现货为 0。",
      evidence: {
        nodes: [
    ...
    expect(screen.getByRole("button", { name: "执行链路" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  });
  ...
  it("routes unbound function output to the recorded execution instead of presenting it as an answer conclusion", () => {
    const functions: EvidenceChainView = {
      ...
      execution: {
        nodes: [
          { id: "op1", label: "First calculation", kind: "function" },
          { id: "op2", label: "Second calculation", kind: "function" },
        ],
        edges: [],
      },
      evidence: {
        nodes: [
          { id: "s1", label: "First source", kind: "source", executionNodeId: "op1" },
          { id: "s2", label: "Second source", kind: "source", executionNodeId: "op2" },
          { id: "f1", label: "Delay", kind: "field", value: "24", executionNodeId: "op1" },
          { id: "f2", label: "Delay", kind: "field", value: "19", executionNodeId: "op2" },
        ],
        edges: [
          { id: "e1", source: "s1", target: "f1", kind: "value", label: "Returned" },
          { id: "e2", source: "s2", target: "f2", kind: "value", label: "Returned" },
        ],
      },
    };
  ...
  it("navigates source to execution and back only through supplied references", () => {
    const evidence = {
      ...view.evidence,
      nodes: view.evidence.nodes.map((n) =>
        n.id === "date" ? { ...n, kind: "source" as const, executionNodeId: "call" } : n,
      ),
    };
    render(<EvidenceChainPanels view={{ ...view, evidence }} />);
  ...
  it("does not link candidates even when their text matches", () => {
    render(
      <EvidenceChainPanels
        view={{ ...view, claims: [{ ...view.claims[0], status: "candidate" }] }}
      />,
    );
  ...
  it("supports Unicode offsets and safely traverses explicit cycles", () => {
    const cyclic = {
      ...
      edges: [
        ...view.evidence.edges,
        { id: "back", source: "delay", target: "date", label: "Recorded reverse link" },
      ],
    };
    render(
      <EvidenceChainPanels
        view={{
          ...
          answer: "😀24 days",
          evidence: cyclic,
          claims: [{ ...view.claims[0], answerRange: { start: 1, end: 8, exact: "24 days" } }],
        }}
      />,
    );
  ...
  it("clears selection when interaction changes and distinguishes missing answer", () => {
    ...
    rerender(
      <EvidenceChainPanels
        view={{ ...view, interactionId: "next", answer: undefined, claims: [] }}
      />,
    );
    expect(screen.getByText("未记录答案正文")).toBeInTheDocument();
    expect(
      screen.queryByText("该结论尚未绑定依据，不能从相同数值推断来源。"),
    ).not.toBeInTheDocument();
  });
});
