...
const graph: ChainGraph = {
  nodes: [
    ...
    { id: "a1", kind: "field", label: "A first" },
    { id: "a2", kind: "field", label: "A second" },
    { id: "b1", kind: "field", label: "B first" },
...
describe("local fact scope", () => {
  ...
  it("never treats unknown edges as object membership", () => {
    expect(
      scopeObjectFacts(
        { ...graph, edges: graph.edges.map((e) => ({ ...e, kind: undefined })) },
        "a",
      ).nodes.map((n) => n.id),
    ).toEqual(["a"]);
  });
...
describe("source-only returned facts", () => {
  const returns: ChainGraph = {
    nodes: [
      { id: "s1", kind: "source", label: "Same function" },
      { id: "s2", kind: "source", label: "Same function" },
      { id: "f1", kind: "field", label: "Delay", value: "24" },
      { id: "f2", kind: "field", label: "Delay", value: "19" },
    ],
    edges: [
      { id: "r1", source: "s1", target: "f1", kind: "value", label: "Returned" },
      { id: "r2", source: "s2", target: "f2", kind: "value", label: "Returned" },
    ],
  };
  ...
  it("keeps returned relationship nodes and their explicit endpoints", () => {
    const related: ChainGraph = {
      nodes: [...graph.nodes, { id: "rel", kind: "relation", label: "Uses" }],
      edges: [
        ...graph.edges,
        { id: "from", source: "a", target: "rel", label: "From", kind: "relation" },
        { id: "to", source: "rel", target: "b", label: "To", kind: "relation" },
      ],
    };
    expect(scopeObjectFacts(related, "a").nodes.map((n) => n.id)).toEqual([
      "a",
      "b",
      "a1",
      "a2",
